 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/TransactionApi/#gettransactions
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->transaction()->getTransactions(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->transaction()->getTransactions(): %s\n", 
        $exc->getMessage()
    );
}