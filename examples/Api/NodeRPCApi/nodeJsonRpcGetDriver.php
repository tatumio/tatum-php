 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/NodeRPCApi/#nodejsonrpcgetdriver
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->nodeRPC()->nodeJsonRpcGetDriver(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->nodeRPC()->nodeJsonRpcGetDriver(): %s\n", 
        $exc->getMessage()
    );
}