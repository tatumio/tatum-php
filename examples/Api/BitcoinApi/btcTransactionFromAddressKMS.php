 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/BitcoinApi/#btctransactionfromaddresskms
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->bitcoin()->btcTransactionFromAddressKMS(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->bitcoin()->btcTransactionFromAddressKMS(): %s\n", 
        $exc->getMessage()
    );
}