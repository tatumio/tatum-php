 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/XRPApi/#transferxrpblockchainassetkms
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->xRP()->transferXrpBlockchainAssetKMS(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->xRP()->transferXrpBlockchainAssetKMS(): %s\n", 
        $exc->getMessage()
    );
}