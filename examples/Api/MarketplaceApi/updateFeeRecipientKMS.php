 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/MarketplaceApi/#updatefeerecipientkms
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->marketplace()->updateFeeRecipientKMS(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->marketplace()->updateFeeRecipientKMS(): %s\n", 
        $exc->getMessage()
    );
}