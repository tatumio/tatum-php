 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/BNBSmartChainApi/#bscgenerateaddress
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->bNBSmartChain()->bscGenerateAddress(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->bNBSmartChain()->bscGenerateAddress(): %s\n", 
        $exc->getMessage()
    );
}