 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/PolygonApi/#polygongettransactionbyaddress
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->polygon()->polygonGetTransactionByAddress(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->polygon()->polygonGetTransactionByAddress(): %s\n", 
        $exc->getMessage()
    );
}