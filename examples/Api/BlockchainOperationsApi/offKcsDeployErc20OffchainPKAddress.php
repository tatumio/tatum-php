 ...
 * @license MIT
 * @author  Mark Jivko
 * @see     SDK docs: https://github.com/tatumio/tatum-php/tree/master/docs/Api/BlockchainOperationsApi.md#offkcsdeployerc20offchainpkaddress
 * 
 * SECURITY WARNING
...
try {
    ...
    $response = $sdk->mainnet()
        ...
        ->offKcsDeployErc20OffchainPKAddress($arg_deploy_erc20_offchain_pk_address);

    var_dump($response);

} catch (\Tatum\Sdk\ApiException $apiExc) {
    echo sprintf(
        "API Exception when calling api()->blockchainOperations()->offKcsDeployErc20OffchainPKAddress(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->blockchainOperations()->offKcsDeployErc20OffchainPKAddress(): %s\n", 
        $exc->getMessage()
    );
}