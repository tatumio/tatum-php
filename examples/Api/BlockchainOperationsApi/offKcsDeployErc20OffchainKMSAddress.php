 ...
 * 
 * @see     SDK docs: https://github.com/tatumio/tatum-php/tree/master/docs/Api/BlockchainOperationsApi.md#offkcsdeployerc20offchainkmsaddress
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    $response = $sdk->mainnet()
        ...
        ->offKcsDeployErc20OffchainKMSAddress($arg_deploy_erc20_offchain_kms_address);

    var_dump($response);

} catch (\Tatum\Sdk\ApiException $apiExc) {
    echo sprintf(
        "API Exception when calling api()->blockchainOperations()->offKcsDeployErc20OffchainKMSAddress(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->blockchainOperations()->offKcsDeployErc20OffchainKMSAddress(): %s\n", 
        $exc->getMessage()
    );
}