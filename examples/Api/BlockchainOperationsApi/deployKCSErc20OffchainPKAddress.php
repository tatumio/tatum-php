...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->blockchainOperations()->deployKCSErc20OffchainPKAddress(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->blockchainOperations()->deployKCSErc20OffchainPKAddress(): %s\n", 
        $exc->getMessage()
    );
}