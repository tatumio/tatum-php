 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/BlockchainOperationsApi/#offxdcdeployerc20offchainmnemonicaddress
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->blockchainOperations()->offXdcDeployErc20OffchainMnemonicAddress(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->blockchainOperations()->offXdcDeployErc20OffchainMnemonicAddress(): %s\n", 
        $exc->getMessage()
    );
}