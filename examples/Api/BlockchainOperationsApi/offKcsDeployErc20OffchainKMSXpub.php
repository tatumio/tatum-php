 ...
 * @license MIT
 * @author  Mark Jivko
 * @see     SDK docs: https://github.com/tatumio/tatum-php/tree/master/docs/Api/BlockchainOperationsApi.md#offkcsdeployerc20offchainkmsxpub
 * 
 * SECURITY WARNING
...
try {
    ...
    $response = $sdk->mainnet()
        ...
        ->offKcsDeployErc20OffchainKMSXpub($arg_deploy_erc20_offchain_kms_xpub);

    var_dump($response);

} catch (\Tatum\Sdk\ApiException $apiExc) {
    echo sprintf(
        "API Exception when calling api()->blockchainOperations()->offKcsDeployErc20OffchainKMSXpub(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->blockchainOperations()->offKcsDeployErc20OffchainKMSXpub(): %s\n", 
        $exc->getMessage()
    );
}