 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/BlockchainOperationsApi/#transferxlmkms
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->blockchainOperations()->transferXlmKMS(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->blockchainOperations()->transferXlmKMS(): %s\n", 
        $exc->getMessage()
    );
}