 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/BlockchainOperationsApi/#offkladeployerc20offchainpkaddress
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    $response = $sdk->mainnet()
        ...
        ->offKlaDeployErc20OffchainPKAddress($arg_deploy_erc20_offchain_pk_address);

    var_dump($response);

} catch (\Tatum\Sdk\ApiException $apiExc) {
    echo sprintf(
        "API Exception when calling api()->blockchainOperations()->offKlaDeployErc20OffchainPKAddress(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->blockchainOperations()->offKlaDeployErc20OffchainPKAddress(): %s\n", 
        $exc->getMessage()
    );
}