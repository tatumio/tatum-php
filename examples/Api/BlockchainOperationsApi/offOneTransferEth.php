 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/BlockchainOperationsApi/#offonetransfereth
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->blockchainOperations()->offOneTransferEth(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->blockchainOperations()->offOneTransferEth(): %s\n", 
        $exc->getMessage()
    );
}