 ...
 * @license MIT
 * @author  Mark Jivko
 * @see     SDK docs: https://github.com/tatumio/tatum-php/tree/master/docs/Api/BlockchainOperationsApi.md#transferdogekeypair
 * 
 * SECURITY WARNING
...
$arg_transfer_doge_key_pair = (new \Tatum\Model\TransferDogeKeyPair())
    ...
    // (optional) Fee to be submitted as a transaction fee to blockchain. If none is set, default value of 1 DOGE is used.
    ->setFee('1')
...
try {
    ...
    $response = $sdk->mainnet()
        ...
        ->transferDogeKeyPair($arg_transfer_doge_key_pair);

    var_dump($response);
} catch (\Tatum\Sdk\ApiException $apiExc) {
    echo "API Exception when calling api()->blockchainOperations()->transferDogeKeyPair(): ", var_export($apiExc->getResponseObject(), true), PHP_EOL;
} catch (\Exception $exc) {
    echo "Exception when calling api()->blockchainOperations()->transferDogeKeyPair(): " . $exc->getMessage() . PHP_EOL;
}