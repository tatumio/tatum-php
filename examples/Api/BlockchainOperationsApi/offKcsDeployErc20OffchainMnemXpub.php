 ...
 * 
 * @see     SDK docs: https://github.com/tatumio/tatum-php/tree/master/docs/Api/BlockchainOperationsApi.md#offkcsdeployerc20offchainmnemxpub
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    $response = $sdk->mainnet()
        ...
        ->offKcsDeployErc20OffchainMnemXpub($arg_deploy_erc20_offchain_mnem_xpub);

    var_dump($response);

} catch (\Tatum\Sdk\ApiException $apiExc) {
    echo sprintf(
        "API Exception when calling api()->blockchainOperations()->offKcsDeployErc20OffchainMnemXpub(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->blockchainOperations()->offKcsDeployErc20OffchainMnemXpub(): %s\n", 
        $exc->getMessage()
    );
}