 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/FlowApi/#flowgeneratepubkey
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->flow()->flowGeneratePubKey(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->flow()->flowGeneratePubKey(): %s\n", 
        $exc->getMessage()
    );
}