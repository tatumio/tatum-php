 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/AuctionApi/#cancelauctionkms
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->auction()->cancelAuctionKMS(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->auction()->cancelAuctionKMS(): %s\n", 
        $exc->getMessage()
    );
}