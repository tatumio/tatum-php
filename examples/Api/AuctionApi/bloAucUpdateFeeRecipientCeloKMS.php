 ...
 * 
 * @link    https://tatumio.github.io/tatum-php/Api/AuctionApi/#bloaucupdatefeerecipientcelokms
 * @license MIT
 * @author  Mark Jivko
 * 
 * SECURITY WARNING
...
try {
    ...
    echo sprintf(
        "API Exception when calling api()->auction()->bloAucUpdateFeeRecipientCeloKMS(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->auction()->bloAucUpdateFeeRecipientCeloKMS(): %s\n", 
        $exc->getMessage()
    );
}