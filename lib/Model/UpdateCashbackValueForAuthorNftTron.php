...
class UpdateCashbackValueForAuthorNftTron extends AbstractModel {
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "token_id"=>null, "contract_address"=>null, "cashback_value"=>null, "fee_limit"=>null, "from_private_key"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("UpdateCashbackValueForAuthorNftTron.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setTokenId(string $token_id) {
        if ((mb_strlen($token_id) > 78)) {
            throw new IAE('UpdateCashbackValueForAuthorNftTron.setTokenId: $token_id length must be <= 78');
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 34)) {
            throw new IAE('UpdateCashbackValueForAuthorNftTron.setContractAddress: $contract_address length must be <= 34');
        }
        if ((mb_strlen($contract_address) < 34)) {
            throw new IAE('UpdateCashbackValueForAuthorNftTron.setContractAddress: $contract_address length must be >= 34');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 64)) {
            throw new IAE('UpdateCashbackValueForAuthorNftTron.setFromPrivateKey: $from_private_key length must be <= 64');
        }
        if ((mb_strlen($from_private_key) < 64)) {
            throw new IAE('UpdateCashbackValueForAuthorNftTron.setFromPrivateKey: $from_private_key length must be >= 64');
        }
