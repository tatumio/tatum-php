...
class CancelSellAssetOnMarketplaceSolanaKMS extends AbstractModel {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("CancelSellAssetOnMarketplaceSolanaKMS.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 44)) {
            throw new IAE('CancelSellAssetOnMarketplaceSolanaKMS.setContractAddress: $contract_address length must be <= 44');
        }
        if ((mb_strlen($contract_address) < 44)) {
            throw new IAE('CancelSellAssetOnMarketplaceSolanaKMS.setContractAddress: $contract_address length must be >= 44');
        }
    ...
    public function setListingId(mixed $listing_id) {
        if ((mb_strlen($listing_id) > 44)) {
            throw new IAE('CancelSellAssetOnMarketplaceSolanaKMS.setListingId: $listing_id length must be <= 44');
        }
        if ((mb_strlen($listing_id) < 44)) {
            throw new IAE('CancelSellAssetOnMarketplaceSolanaKMS.setListingId: $listing_id length must be >= 44');
        }
    ...
    public function setFrom(mixed $from) {
        if ((mb_strlen($from) > 44)) {
            throw new IAE('CancelSellAssetOnMarketplaceSolanaKMS.setFrom: $from length must be <= 44');
        }
        if ((mb_strlen($from) < 44)) {
            throw new IAE('CancelSellAssetOnMarketplaceSolanaKMS.setFrom: $from length must be >= 44');
        }
