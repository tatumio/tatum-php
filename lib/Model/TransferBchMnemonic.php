 ...
 * TransferBchMnemonic Model
 */
class TransferBchMnemonic extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "TransferBchMnemonic";
    ...
    public function __construct(array $data = []) {
        foreach(["sender_account_id"=>null, "address"=>null, "amount"=>null, "compliant"=>null, "multiple_amounts"=>null, "fee"=>null, "attr"=>null, "mnemonic"=>null, "xpub"=>null, "payment_id"=>null, "sender_note"=>null] as $k => $v) {
    ...
    public function setSenderAccountId(string $sender_account_id) {
        if ((mb_strlen($sender_account_id) > 24)) {
            throw new IAE('TransferBchMnemonic.setSenderAccountId: $sender_account_id length must be <= 24');
        }
        if ((mb_strlen($sender_account_id) < 24)) {
            throw new IAE('TransferBchMnemonic.setSenderAccountId: $sender_account_id length must be >= 24');
        }
    ...
    public function setAddress(string $address) {
        if ((mb_strlen($address) > 10000)) {
            throw new IAE('TransferBchMnemonic.setAddress: $address length must be <= 10000');
        }
        if ((mb_strlen($address) < 1)) {
            throw new IAE('TransferBchMnemonic.setAddress: $address length must be >= 1');
        }
    ...
    public function setAmount(string $amount) {
        if ((mb_strlen($amount) > 38)) {
            throw new IAE('TransferBchMnemonic.setAmount: $amount length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('TransferBchMnemonic.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFee(?string $fee) {
        if (!is_null($fee) && (!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $fee))) {
            throw new IAE('TransferBchMnemonic.setFee: $fee must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($fee, true) . ' given');
        }
        $this->_data['fee'] = $fee;
    ...
    public function setAttr(?string $attr) {
        if (!is_null($attr) && (mb_strlen($attr) > 256)) {
            throw new IAE('TransferBchMnemonic.setAttr: $attr length must be <= 256');
        }
        if (!is_null($attr) && (mb_strlen($attr) < 1)) {
            throw new IAE('TransferBchMnemonic.setAttr: $attr length must be >= 1');
        }
    ...
    public function setMnemonic(string $mnemonic) {
        if ((mb_strlen($mnemonic) > 500)) {
            throw new IAE('TransferBchMnemonic.setMnemonic: $mnemonic length must be <= 500');
        }
        if ((mb_strlen($mnemonic) < 1)) {
            throw new IAE('TransferBchMnemonic.setMnemonic: $mnemonic length must be >= 1');
        }
    ...
    public function setXpub(string $xpub) {
        if ((mb_strlen($xpub) > 150)) {
            throw new IAE('TransferBchMnemonic.setXpub: $xpub length must be <= 150');
        }
        if ((mb_strlen($xpub) < 1)) {
            throw new IAE('TransferBchMnemonic.setXpub: $xpub length must be >= 1');
        }
    ...
    public function setPaymentId(?string $payment_id) {
        if (!is_null($payment_id) && (mb_strlen($payment_id) > 100)) {
            throw new IAE('TransferBchMnemonic.setPaymentId: $payment_id length must be <= 100');
        }
        if (!is_null($payment_id) && (mb_strlen($payment_id) < 1)) {
            throw new IAE('TransferBchMnemonic.setPaymentId: $payment_id length must be >= 1');
        }
    ...
    public function setSenderNote(?string $sender_note) {
        if (!is_null($sender_note) && (mb_strlen($sender_note) > 500)) {
            throw new IAE('TransferBchMnemonic.setSenderNote: $sender_note length must be <= 500');
        }
        if (!is_null($sender_note) && (mb_strlen($sender_note) < 1)) {
            throw new IAE('TransferBchMnemonic.setSenderNote: $sender_note length must be >= 1');
        }
