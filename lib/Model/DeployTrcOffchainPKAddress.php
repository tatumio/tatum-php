...
/**
 * DeployTrcOffchainPKAddress Model
 *
 * @version   3.17.1
 ...
 * DeployTrcOffchainPKAddress Model
 */
class DeployTrcOffchainPKAddress extends AbstractModel {
    ...
    protected static $_name = "DeployTrcOffchainPKAddress";
    ...
    public function __construct(array $data = []) {
        foreach(["symbol"=>null, "supply"=>null, "decimals"=>null, "type"=>null, "description"=>null, "url"=>null, "base_pair"=>null, "base_rate"=>1, "customer"=>null, "address"=>null, "private_key"=>null] as $k => $v) {
    ...
    public function setSymbol(string $symbol) {
        if ((mb_strlen($symbol) > 30)) {
            throw new IAE('DeployTrcOffchainPKAddress.setSymbol: $symbol length must be <= 30');
        }
        if ((mb_strlen($symbol) < 1)) {
            throw new IAE('DeployTrcOffchainPKAddress.setSymbol: $symbol length must be >= 1');
        }
        if ((!preg_match("/^[a-zA-Z0-9_]+$/", $symbol))) {
            throw new IAE('DeployTrcOffchainPKAddress.setSymbol: $symbol must match /^[a-zA-Z0-9_]+$/, ' . var_export($symbol, true) . ' given');
        }
    ...
    public function setSupply(string $supply) {
        if ((mb_strlen($supply) > 38)) {
            throw new IAE('DeployTrcOffchainPKAddress.setSupply: $supply length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $supply))) {
            throw new IAE('DeployTrcOffchainPKAddress.setSupply: $supply must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($supply, true) . ' given');
        }
    ...
    public function setDecimals(float $decimals) {
        if (($decimals < 0)) {
            throw new IAE('DeployTrcOffchainPKAddress.setDecimals: $decimals must be >=0');
        }
    ...
    public function setType(string $type) {
        ...
        if (!in_array($type, $allowed, true)) {
            throw new IAE(sprintf("DeployTrcOffchainPKAddress.setType: type invalid value '%s', must be one of '%s'", $type, implode("', '", $allowed)));
        }
    ...
    public function setDescription(string $description) {
        if ((mb_strlen($description) > 100)) {
            throw new IAE('DeployTrcOffchainPKAddress.setDescription: $description length must be <= 100');
        }
        if ((mb_strlen($description) < 1)) {
            throw new IAE('DeployTrcOffchainPKAddress.setDescription: $description length must be >= 1');
        }
    ...
    public function setUrl(?string $url) {
        if (!is_null($url) && (mb_strlen($url) > 100)) {
            throw new IAE('DeployTrcOffchainPKAddress.setUrl: $url length must be <= 100');
        }
        if (!is_null($url) && (mb_strlen($url) < 1)) {
            throw new IAE('DeployTrcOffchainPKAddress.setUrl: $url length must be >= 1');
        }
        $this->_data['url'] = $url;
    ...
    public function setBasePair(string $base_pair) {
        ...
        if (!in_array($base_pair, $allowed, true)) {
            throw new IAE(sprintf("DeployTrcOffchainPKAddress.setBasePair: base_pair invalid value '%s', must be one of '%s'", $base_pair, implode("', '", $allowed)));
        }
        if ((mb_strlen($base_pair) > 30)) {
            throw new IAE('DeployTrcOffchainPKAddress.setBasePair: $base_pair length must be <= 30');
        }
        if ((mb_strlen($base_pair) < 2)) {
            throw new IAE('DeployTrcOffchainPKAddress.setBasePair: $base_pair length must be >= 2');
        }
    ...
    public function setBaseRate(?float $base_rate) {
        if (!is_null($base_rate) && ($base_rate < 0)) {
            throw new IAE('DeployTrcOffchainPKAddress.setBaseRate: $base_rate must be >=0');
        }
    ...
    public function setAddress(string $address) {
        if ((mb_strlen($address) > 42)) {
            throw new IAE('DeployTrcOffchainPKAddress.setAddress: $address length must be <= 42');
        }
        if ((mb_strlen($address) < 42)) {
            throw new IAE('DeployTrcOffchainPKAddress.setAddress: $address length must be >= 42');
        }
    ...
    public function setPrivateKey(string $private_key) {
        if ((mb_strlen($private_key) > 66)) {
            throw new IAE('DeployTrcOffchainPKAddress.setPrivateKey: $private_key length must be <= 66');
        }
        if ((mb_strlen($private_key) < 66)) {
            throw new IAE('DeployTrcOffchainPKAddress.setPrivateKey: $private_key length must be >= 66');
        }
