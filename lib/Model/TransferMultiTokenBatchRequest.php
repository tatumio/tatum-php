...
/**
 * TransferMultiTokenBatch_request Model
 *
 * @version   3.17.1
 ...
 * TransferMultiTokenBatch_request Model
 */
class TransferMultiTokenBatchRequest extends AbstractModel {
    ...
    protected static $_name = "TransferMultiTokenBatch_request";
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "to"=>null, "token_id"=>null, "amounts"=>null, "data"=>null, "contract_address"=>null, "from_private_key"=>null, "nonce"=>null, "fee"=>null, "amount"=>null, "index"=>null, "signature_id"=>null, "fee_currency"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("TransferMultiTokenBatchRequest.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 42)) {
            throw new IAE('TransferMultiTokenBatchRequest.setTo: $to length must be <= 42');
        }
        if ((mb_strlen($to) < 42)) {
            throw new IAE('TransferMultiTokenBatchRequest.setTo: $to length must be >= 42');
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('TransferMultiTokenBatchRequest.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('TransferMultiTokenBatchRequest.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('TransferMultiTokenBatchRequest.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('TransferMultiTokenBatchRequest.setFromPrivateKey: $from_private_key length must be >= 66');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('TransferMultiTokenBatchRequest.setIndex: $index must be >=0');
        }
    ...
    public function setFeeCurrency(string $fee_currency) {
        ...
        if (!in_array($fee_currency, $allowed, true)) {
            throw new IAE(sprintf("TransferMultiTokenBatchRequest.setFeeCurrency: fee_currency invalid value '%s', must be one of '%s'", $fee_currency, implode("', '", $allowed)));
        }
