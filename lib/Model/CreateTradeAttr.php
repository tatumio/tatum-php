 ...
 */
class CreateTradeAttr extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "CreateTrade_attr";
    ...
    public function setSealDate(float $seal_date) {
        if (($seal_date < 0)) {
            throw new IAE('CreateTradeAttr.setSealDate: $seal_date must be >=0');
        }
    ...
    public function setPercentBlock(?mixed $percent_block) {
        if (!is_null($percent_block) && ($percent_block > 100)) {
            throw new IAE('CreateTradeAttr.setPercentBlock: $percent_block must be <=100');
        }
        if (!is_null($percent_block) && ($percent_block < 0)) {
            throw new IAE('CreateTradeAttr.setPercentBlock: $percent_block must be >=0');
        }
    ...
    public function setPercentPenalty(?mixed $percent_penalty) {
        if (!is_null($percent_penalty) && ($percent_penalty > 100)) {
            throw new IAE('CreateTradeAttr.setPercentPenalty: $percent_penalty must be <=100');
        }
        if (!is_null($percent_penalty) && ($percent_penalty < 0)) {
            throw new IAE('CreateTradeAttr.setPercentPenalty: $percent_penalty must be >=0');
        }
