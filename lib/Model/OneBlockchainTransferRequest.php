...
/**
 * OneBlockchainTransfer_request Model
 *
 * @version   3.17.0
 ...
 * OneBlockchainTransfer_request Model
 */
class OneBlockchainTransferRequest extends AbstractModel {

    public const DISCRIMINATOR = null;
    public const CURRENCY_ONE = 'ONE';
    protected static $_name = "OneBlockchainTransfer_request";
    ...
    public function setData(?string $data) {
        if (!is_null($data) && (mb_strlen($data) > 50000)) {
            throw new IAE('OneBlockchainTransferRequest.setData: $data length must be <= 50000');
        }
    ...
    public function setCurrency(string $currency) {
        ...
        if (!in_array($currency, $allowed, true)) {
            throw new IAE(sprintf("OneBlockchainTransferRequest.setCurrency: currency invalid value '%s', must be one of '%s'", $currency, implode("', '", $allowed)));
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('OneBlockchainTransferRequest.setNonce: $nonce must be >=0');
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 42)) {
            throw new IAE('OneBlockchainTransferRequest.setTo: $to length must be <= 42');
        }
        if ((mb_strlen($to) < 42)) {
            throw new IAE('OneBlockchainTransferRequest.setTo: $to length must be >= 42');
        }
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('OneBlockchainTransferRequest.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('OneBlockchainTransferRequest.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('OneBlockchainTransferRequest.setFromPrivateKey: $from_private_key length must be >= 66');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('OneBlockchainTransferRequest.setIndex: $index must be >=0');
        }
