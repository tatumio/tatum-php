...
class BuyAssetOnMarketplaceCelo extends AbstractModel {
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "fee_currency"=>null, "contract_address"=>null, "erc20_address"=>null, "listing_id"=>null, "buyer"=>null, "amount"=>null, "from_private_key"=>null, "nonce"=>null, "fee"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("BuyAssetOnMarketplaceCelo.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setFeeCurrency(string $fee_currency) {
        ...
        if (!in_array($fee_currency, $allowed, true)) {
            throw new IAE(sprintf("BuyAssetOnMarketplaceCelo.setFeeCurrency: fee_currency invalid value '%s', must be one of '%s'", $fee_currency, implode("', '", $allowed)));
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setErc20Address(?string $erc20_address) {
        if (!is_null($erc20_address) && (mb_strlen($erc20_address) > 42)) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setErc20Address: $erc20_address length must be <= 42');
        }
        if (!is_null($erc20_address) && (mb_strlen($erc20_address) < 42)) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setErc20Address: $erc20_address length must be >= 42');
        }
    ...
    public function setListingId(string $listing_id) {
        if ((mb_strlen($listing_id) > 200)) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setListingId: $listing_id length must be <= 200');
        }
        if ((mb_strlen($listing_id) < 1)) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setListingId: $listing_id length must be >= 1');
        }
    ...
    public function setBuyer(?string $buyer) {
        if (!is_null($buyer) && (mb_strlen($buyer) > 42)) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setBuyer: $buyer length must be <= 42');
        }
        if (!is_null($buyer) && (mb_strlen($buyer) < 42)) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setBuyer: $buyer length must be >= 42');
        }
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('BuyAssetOnMarketplaceCelo.setFromPrivateKey: $from_private_key length must be >= 66');
        }
