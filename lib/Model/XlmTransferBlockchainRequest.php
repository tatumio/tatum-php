...
/**
 * XlmTransferBlockchain_request Model
 *
 * @version   3.17.1
 ...
 * XlmTransferBlockchain_request Model
 */
class XlmTransferBlockchainRequest extends AbstractModel {
    ...
    public function setFromAccount(string $from_account) {
        if ((mb_strlen($from_account) > 56)) {
            throw new IAE('XlmTransferBlockchainRequest.setFromAccount: $from_account length must be <= 56');
        }
        if ((mb_strlen($from_account) < 56)) {
            throw new IAE('XlmTransferBlockchainRequest.setFromAccount: $from_account length must be >= 56');
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 56)) {
            throw new IAE('XlmTransferBlockchainRequest.setTo: $to length must be <= 56');
        }
        if ((mb_strlen($to) < 56)) {
            throw new IAE('XlmTransferBlockchainRequest.setTo: $to length must be >= 56');
        }
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('XlmTransferBlockchainRequest.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFromSecret(string $from_secret) {
        if ((mb_strlen($from_secret) > 56)) {
            throw new IAE('XlmTransferBlockchainRequest.setFromSecret: $from_secret length must be <= 56');
        }
        if ((mb_strlen($from_secret) < 56)) {
            throw new IAE('XlmTransferBlockchainRequest.setFromSecret: $from_secret length must be >= 56');
        }
    ...
    public function setMessage(?string $message) {
        if (!is_null($message) && (mb_strlen($message) > 64)) {
            throw new IAE('XlmTransferBlockchainRequest.setMessage: $message length must be <= 64');
        }
        if (!is_null($message) && (!preg_match("/^[ -~]{0,64}$/", $message))) {
            throw new IAE('XlmTransferBlockchainRequest.setMessage: $message must match /^[ -~]{0,64}$/, ' . var_export($message, true) . ' given');
        }
    ...
    public function setToken(string $token) {
        if ((mb_strlen($token) > 12)) {
            throw new IAE('XlmTransferBlockchainRequest.setToken: $token length must be <= 12');
        }
        if ((mb_strlen($token) < 1)) {
            throw new IAE('XlmTransferBlockchainRequest.setToken: $token length must be >= 1');
        }
        if ((!preg_match("/^[a-zA-Z0-9]{1,12}$/", $token))) {
            throw new IAE('XlmTransferBlockchainRequest.setToken: $token must match /^[a-zA-Z0-9]{1,12}$/, ' . var_export($token, true) . ' given');
        }
    ...
    public function setIssuerAccount(string $issuer_account) {
        if ((mb_strlen($issuer_account) > 56)) {
            throw new IAE('XlmTransferBlockchainRequest.setIssuerAccount: $issuer_account length must be <= 56');
        }
        if ((mb_strlen($issuer_account) < 56)) {
            throw new IAE('XlmTransferBlockchainRequest.setIssuerAccount: $issuer_account length must be >= 56');
        }
