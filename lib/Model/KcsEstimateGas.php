 ...
 * KcsEstimateGas Model
 */
class KcsEstimateGas extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "KcsEstimateGas";
    ...
    public function setFrom(string $from) {
        if ((mb_strlen($from) > 42)) {
            throw new IAE('KcsEstimateGas.setFrom: $from length must be <= 42');
        }
        if ((mb_strlen($from) < 42)) {
            throw new IAE('KcsEstimateGas.setFrom: $from length must be >= 42');
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 42)) {
            throw new IAE('KcsEstimateGas.setTo: $to length must be <= 42');
        }
        if ((mb_strlen($to) < 42)) {
            throw new IAE('KcsEstimateGas.setTo: $to length must be >= 42');
        }
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('KcsEstimateGas.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setData(?string $data) {
        if (!is_null($data) && (mb_strlen($data) > 50000)) {
            throw new IAE('KcsEstimateGas.setData: $data length must be <= 50000');
        }
        if (!is_null($data) && (!preg_match("/^(0x|0h)?[0-9A-F]+$/", $data))) {
            throw new IAE('KcsEstimateGas.setData: $data must match /^(0x|0h)?[0-9A-F]+$/, ' . var_export($data, true) . ' given');
        }
