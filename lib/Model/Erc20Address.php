 ...
 * Erc20Address Model
 */
class Erc20Address extends AbstractModel {
    ...
    protected static $_name = "Erc20Address";
    ...
    public function setSymbol(string $symbol) {
        if ((mb_strlen($symbol) > 30)) {
            throw new IAE('Erc20Address.setSymbol: $symbol length must be <= 30');
        }
        if ((mb_strlen($symbol) < 1)) {
            throw new IAE('Erc20Address.setSymbol: $symbol length must be >= 1');
        }
        if ((!preg_match("/^[a-zA-Z0-9_]+$/", $symbol))) {
            throw new IAE('Erc20Address.setSymbol: $symbol must match /^[a-zA-Z0-9_]+$/, ' . var_export($symbol, true) . ' given');
        }
    ...
    public function setSupply(string $supply) {
        if ((mb_strlen($supply) > 38)) {
            throw new IAE('Erc20Address.setSupply: $supply length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $supply))) {
            throw new IAE('Erc20Address.setSupply: $supply must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($supply, true) . ' given');
        }
    ...
    public function setDecimals(float $decimals) {
        if (($decimals < 0)) {
            throw new IAE('Erc20Address.setDecimals: $decimals must be >=0');
        }
    ...
    public function setDescription(string $description) {
        if ((mb_strlen($description) > 100)) {
            throw new IAE('Erc20Address.setDescription: $description length must be <= 100');
        }
    ...
    public function setBasePair(string $base_pair) {
        ...
        if (!in_array($base_pair, $allowed, true)) {
            throw new IAE(sprintf("Erc20Address.setBasePair: base_pair invalid value '%s', must be one of '%s'", $base_pair, implode("', '", $allowed)));
        }
        if ((mb_strlen($base_pair) > 50)) {
            throw new IAE('Erc20Address.setBasePair: $base_pair length must be <= 50');
        }
        if ((mb_strlen($base_pair) < 3)) {
            throw new IAE('Erc20Address.setBasePair: $base_pair length must be >= 3');
        }
    ...
    public function setBaseRate(?float $base_rate) {
        if (!is_null($base_rate) && ($base_rate < 0)) {
            throw new IAE('Erc20Address.setBaseRate: $base_rate must be >=0');
        }
    ...
    public function setAccountingCurrency(?string $accounting_currency) {
        ...
        if (!is_null($accounting_currency) && !in_array($accounting_currency, $allowed, true)) {
            throw new IAE(sprintf("Erc20Address.setAccountingCurrency: accounting_currency invalid value '%s', must be one of '%s'", $accounting_currency, implode("', '", $allowed)));
        }
