...
class BuyAssetOnMarketplace extends AbstractModel {
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "contract_address"=>null, "erc20_address"=>null, "buyer"=>null, "listing_id"=>null, "amount"=>null, "from_private_key"=>null, "nonce"=>null, "fee"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("BuyAssetOnMarketplace.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('BuyAssetOnMarketplace.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('BuyAssetOnMarketplace.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setErc20Address(?string $erc20_address) {
        if (!is_null($erc20_address) && (mb_strlen($erc20_address) > 42)) {
            throw new IAE('BuyAssetOnMarketplace.setErc20Address: $erc20_address length must be <= 42');
        }
        if (!is_null($erc20_address) && (mb_strlen($erc20_address) < 42)) {
            throw new IAE('BuyAssetOnMarketplace.setErc20Address: $erc20_address length must be >= 42');
        }
    ...
    public function setBuyer(?string $buyer) {
        if (!is_null($buyer) && (mb_strlen($buyer) > 42)) {
            throw new IAE('BuyAssetOnMarketplace.setBuyer: $buyer length must be <= 42');
        }
        if (!is_null($buyer) && (mb_strlen($buyer) < 42)) {
            throw new IAE('BuyAssetOnMarketplace.setBuyer: $buyer length must be >= 42');
        }
    ...
    public function setListingId(string $listing_id) {
        if ((mb_strlen($listing_id) > 200)) {
            throw new IAE('BuyAssetOnMarketplace.setListingId: $listing_id length must be <= 200');
        }
        if ((mb_strlen($listing_id) < 1)) {
            throw new IAE('BuyAssetOnMarketplace.setListingId: $listing_id length must be >= 1');
        }
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('BuyAssetOnMarketplace.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('BuyAssetOnMarketplace.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('BuyAssetOnMarketplace.setFromPrivateKey: $from_private_key length must be >= 66');
        }
