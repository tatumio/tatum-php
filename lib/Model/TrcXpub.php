 ...
 * TrcXpub Model
 */
class TrcXpub extends AbstractModel {
    ...
    public function setSymbol(string $symbol) {
        if ((mb_strlen($symbol) > 30)) {
            throw new IAE('TrcXpub.setSymbol: $symbol length must be <= 30');
        }
        if ((mb_strlen($symbol) < 1)) {
            throw new IAE('TrcXpub.setSymbol: $symbol length must be >= 1');
        }
        if ((!preg_match("/^[a-zA-Z0-9_]+$/", $symbol))) {
            throw new IAE('TrcXpub.setSymbol: $symbol must match /^[a-zA-Z0-9_]+$/, ' . var_export($symbol, true) . ' given');
        }
    ...
    public function setSupply(string $supply) {
        if ((mb_strlen($supply) > 38)) {
            throw new IAE('TrcXpub.setSupply: $supply length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $supply))) {
            throw new IAE('TrcXpub.setSupply: $supply must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($supply, true) . ' given');
        }
    ...
    public function setDecimals(float $decimals) {
        if (($decimals < 0)) {
            throw new IAE('TrcXpub.setDecimals: $decimals must be >=0');
        }
    ...
    public function setType(string $type) {
        ...
        if (!in_array($type, $allowed, true)) {
            throw new IAE(sprintf("TrcXpub.setType: type invalid value '%s', must be one of '%s'", $type, implode("', '", $allowed)));
        }
    ...
    public function setDescription(string $description) {
        if ((mb_strlen($description) > 100)) {
            throw new IAE('TrcXpub.setDescription: $description length must be <= 100');
        }
        if ((mb_strlen($description) < 1)) {
            throw new IAE('TrcXpub.setDescription: $description length must be >= 1');
        }
    ...
    public function setXpub(string $xpub) {
        if ((mb_strlen($xpub) > 130)) {
            throw new IAE('TrcXpub.setXpub: $xpub length must be <= 130');
        }
        if ((mb_strlen($xpub) < 130)) {
            throw new IAE('TrcXpub.setXpub: $xpub length must be >= 130');
        }
    ...
    public function setDerivationIndex(int $derivation_index) {
        if (($derivation_index > 2147483647)) {
            throw new IAE('TrcXpub.setDerivationIndex: $derivation_index must be <=2147483647');
        }
    ...
    public function setUrl(string $url) {
        if ((mb_strlen($url) > 100)) {
            throw new IAE('TrcXpub.setUrl: $url length must be <= 100');
        }
        if ((mb_strlen($url) < 1)) {
            throw new IAE('TrcXpub.setUrl: $url length must be >= 1');
        }
        $this->_data['url'] = $url;
    ...
    public function setBasePair(string $base_pair) {
        ...
        if (!in_array($base_pair, $allowed, true)) {
            throw new IAE(sprintf("TrcXpub.setBasePair: base_pair invalid value '%s', must be one of '%s'", $base_pair, implode("', '", $allowed)));
        }
        if ((mb_strlen($base_pair) > 50)) {
            throw new IAE('TrcXpub.setBasePair: $base_pair length must be <= 50');
        }
        if ((mb_strlen($base_pair) < 3)) {
            throw new IAE('TrcXpub.setBasePair: $base_pair length must be >= 3');
        }
    ...
    public function setBaseRate(?float $base_rate) {
        if (!is_null($base_rate) && ($base_rate < 0)) {
            throw new IAE('TrcXpub.setBaseRate: $base_rate must be >=0');
        }
    ...
    public function setAccountingCurrency(?string $accounting_currency) {
        ...
        if (!is_null($accounting_currency) && !in_array($accounting_currency, $allowed, true)) {
            throw new IAE(sprintf("TrcXpub.setAccountingCurrency: accounting_currency invalid value '%s', must be one of '%s'", $accounting_currency, implode("', '", $allowed)));
        }
        if (!is_null($accounting_currency) && (mb_strlen($accounting_currency) > 3)) {
            throw new IAE('TrcXpub.setAccountingCurrency: $accounting_currency length must be <= 3');
        }
        if (!is_null($accounting_currency) && (mb_strlen($accounting_currency) < 3)) {
            throw new IAE('TrcXpub.setAccountingCurrency: $accounting_currency length must be >= 3');
        }
