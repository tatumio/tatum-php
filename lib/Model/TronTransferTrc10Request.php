...
/**
 * TronTransferTrc10_request Model
 *
 * @version   3.17.0
 ...
 * TronTransferTrc10_request Model
 */
class TronTransferTrc10Request extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "TronTransferTrc10_request";
    ...
    public function __construct(array $data = []) {
        foreach(["from_private_key"=>null, "to"=>null, "token_id"=>null, "amount"=>null, "from"=>null, "signature_id"=>null, "index"=>null] as $k => $v) {
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 64)) {
            throw new IAE('TronTransferTrc10Request.setFromPrivateKey: $from_private_key length must be <= 64');
        }
        if ((mb_strlen($from_private_key) < 64)) {
            throw new IAE('TronTransferTrc10Request.setFromPrivateKey: $from_private_key length must be >= 64');
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 34)) {
            throw new IAE('TronTransferTrc10Request.setTo: $to length must be <= 34');
        }
        if ((mb_strlen($to) < 34)) {
            throw new IAE('TronTransferTrc10Request.setTo: $to length must be >= 34');
        }
    ...
    public function setTokenId(string $token_id) {
        if ((mb_strlen($token_id) > 100)) {
            throw new IAE('TronTransferTrc10Request.setTokenId: $token_id length must be <= 100');
        }
        if ((mb_strlen($token_id) < 1)) {
            throw new IAE('TronTransferTrc10Request.setTokenId: $token_id length must be >= 1');
        }
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('TronTransferTrc10Request.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFrom(string $from) {
        if ((mb_strlen($from) > 34)) {
            throw new IAE('TronTransferTrc10Request.setFrom: $from length must be <= 34');
        }
        if ((mb_strlen($from) < 34)) {
            throw new IAE('TronTransferTrc10Request.setFrom: $from length must be >= 34');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('TronTransferTrc10Request.setIndex: $index must be >=0');
        }
