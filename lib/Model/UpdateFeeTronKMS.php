...
class UpdateFeeTronKMS extends AbstractModel {
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "contract_address"=>null, "from"=>null, "signature_id"=>null, "index"=>null, "marketplace_fee"=>null, "from_private_key"=>null, "fee_limit"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("UpdateFeeTronKMS.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 34)) {
            throw new IAE('UpdateFeeTronKMS.setContractAddress: $contract_address length must be <= 34');
        }
        if ((mb_strlen($contract_address) < 34)) {
            throw new IAE('UpdateFeeTronKMS.setContractAddress: $contract_address length must be >= 34');
        }
    ...
    public function setFrom(string $from) {
        if ((mb_strlen($from) > 34)) {
            throw new IAE('UpdateFeeTronKMS.setFrom: $from length must be <= 34');
        }
        if ((mb_strlen($from) < 34)) {
            throw new IAE('UpdateFeeTronKMS.setFrom: $from length must be >= 34');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('UpdateFeeTronKMS.setIndex: $index must be >=0');
        }
    ...
    public function setMarketplaceFee(float $marketplace_fee) {
        if (($marketplace_fee > 10000)) {
            throw new IAE('UpdateFeeTronKMS.setMarketplaceFee: $marketplace_fee must be <=10000');
        }
        if (($marketplace_fee < 0)) {
            throw new IAE('UpdateFeeTronKMS.setMarketplaceFee: $marketplace_fee must be >=0');
        }
    ...
    public function setFromPrivateKey(?string $from_private_key) {
        if (!is_null($from_private_key) && (mb_strlen($from_private_key) > 64)) {
            throw new IAE('UpdateFeeTronKMS.setFromPrivateKey: $from_private_key length must be <= 64');
        }
        if (!is_null($from_private_key) && (mb_strlen($from_private_key) < 64)) {
            throw new IAE('UpdateFeeTronKMS.setFromPrivateKey: $from_private_key length must be >= 64');
        }
    ...
    public function setFeeLimit(float $fee_limit) {
        if (($fee_limit < 0)) {
            throw new IAE('UpdateFeeTronKMS.setFeeLimit: $fee_limit must be >=0');
        }
