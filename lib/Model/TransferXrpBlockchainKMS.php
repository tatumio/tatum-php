...
/**
 * TransferXrpBlockchainKMS Model
 *
 * @version   3.17.0
 ...
 * TransferXrpBlockchainKMS Model
 */
class TransferXrpBlockchainKMS extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "TransferXrpBlockchainKMS";
    ...
    public function __construct(array $data = []) {
        foreach(["from_account"=>null, "to"=>null, "amount"=>null, "signature_id"=>null, "fee"=>null, "source_tag"=>null, "destination_tag"=>null] as $k => $v) {
    ...
    public function setFromAccount(string $from_account) {
        if ((mb_strlen($from_account) > 34)) {
            throw new IAE('TransferXrpBlockchainKMS.setFromAccount: $from_account length must be <= 34');
        }
        if ((mb_strlen($from_account) < 33)) {
            throw new IAE('TransferXrpBlockchainKMS.setFromAccount: $from_account length must be >= 33');
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 34)) {
            throw new IAE('TransferXrpBlockchainKMS.setTo: $to length must be <= 34');
        }
        if ((mb_strlen($to) < 33)) {
            throw new IAE('TransferXrpBlockchainKMS.setTo: $to length must be >= 33');
        }
        $this->_data['to'] = $to;
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('TransferXrpBlockchainKMS.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFee(?string $fee) {
        if (!is_null($fee) && (!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $fee))) {
            throw new IAE('TransferXrpBlockchainKMS.setFee: $fee must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($fee, true) . ' given');
        }
        $this->_data['fee'] = $fee;
