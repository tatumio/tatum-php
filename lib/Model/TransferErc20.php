 ...
 * TransferErc20 Model
 */
class TransferErc20 extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "TransferErc20";
    ...
    public function __construct(array $data = []) {
        foreach(["sender_account_id"=>null, "address"=>null, "amount"=>null, "compliant"=>null, "currency"=>null, "gas_limit"=>null, "gas_price"=>null, "private_key"=>null, "nonce"=>null, "payment_id"=>null, "sender_note"=>null] as $k => $v) {
    ...
    public function setSenderAccountId(string $sender_account_id) {
        if ((mb_strlen($sender_account_id) > 24)) {
            throw new IAE('TransferErc20.setSenderAccountId: $sender_account_id length must be <= 24');
        }
        if ((mb_strlen($sender_account_id) < 24)) {
            throw new IAE('TransferErc20.setSenderAccountId: $sender_account_id length must be >= 24');
        }
    ...
    public function setAddress(string $address) {
        if ((mb_strlen($address) > 42)) {
            throw new IAE('TransferErc20.setAddress: $address length must be <= 42');
        }
        if ((mb_strlen($address) < 42)) {
            throw new IAE('TransferErc20.setAddress: $address length must be >= 42');
        }
    ...
    public function setAmount(string $amount) {
        if ((mb_strlen($amount) > 38)) {
            throw new IAE('TransferErc20.setAmount: $amount length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('TransferErc20.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setCurrency(?string $currency) {
        if (!is_null($currency) && (mb_strlen($currency) > 30)) {
            throw new IAE('TransferErc20.setCurrency: $currency length must be <= 30');
        }
        if (!is_null($currency) && (mb_strlen($currency) < 1)) {
            throw new IAE('TransferErc20.setCurrency: $currency length must be >= 1');
        }
    ...
    public function setGasLimit(?string $gas_limit) {
        if (!is_null($gas_limit) && (!preg_match("/^[+]?\\d+$/", $gas_limit))) {
            throw new IAE('TransferErc20.setGasLimit: $gas_limit must match /^[+]?\\d+$/, ' . var_export($gas_limit, true) . ' given');
        }
    ...
    public function setGasPrice(?string $gas_price) {
        if (!is_null($gas_price) && (!preg_match("/^[+]?\\d+$/", $gas_price))) {
            throw new IAE('TransferErc20.setGasPrice: $gas_price must match /^[+]?\\d+$/, ' . var_export($gas_price, true) . ' given');
        }
    ...
    public function setPrivateKey(string $private_key) {
        if ((mb_strlen($private_key) > 66)) {
            throw new IAE('TransferErc20.setPrivateKey: $private_key length must be <= 66');
        }
        if ((mb_strlen($private_key) < 66)) {
            throw new IAE('TransferErc20.setPrivateKey: $private_key length must be >= 66');
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('TransferErc20.setNonce: $nonce must be >=0');
        }
    ...
    public function setPaymentId(?string $payment_id) {
        if (!is_null($payment_id) && (mb_strlen($payment_id) > 100)) {
            throw new IAE('TransferErc20.setPaymentId: $payment_id length must be <= 100');
        }
        if (!is_null($payment_id) && (mb_strlen($payment_id) < 1)) {
            throw new IAE('TransferErc20.setPaymentId: $payment_id length must be >= 1');
        }
    ...
    public function setSenderNote(?string $sender_note) {
        if (!is_null($sender_note) && (mb_strlen($sender_note) > 500)) {
            throw new IAE('TransferErc20.setSenderNote: $sender_note length must be <= 500');
        }
        if (!is_null($sender_note) && (mb_strlen($sender_note) < 1)) {
            throw new IAE('TransferErc20.setSenderNote: $sender_note length must be >= 1');
        }
