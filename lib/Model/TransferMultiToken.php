 ...
 * TransferMultiToken Model
 */
class TransferMultiToken extends AbstractModel {
    ...
    protected static $_name = "TransferMultiToken";
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "to"=>null, "token_id"=>null, "amount"=>null, "data"=>null, "contract_address"=>null, "from_private_key"=>null, "nonce"=>null, "fee"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("TransferMultiToken.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 42)) {
            throw new IAE('TransferMultiToken.setTo: $to length must be <= 42');
        }
        if ((mb_strlen($to) < 42)) {
            throw new IAE('TransferMultiToken.setTo: $to length must be >= 42');
        }
    ...
    public function setTokenId(string $token_id) {
        if ((mb_strlen($token_id) > 256)) {
            throw new IAE('TransferMultiToken.setTokenId: $token_id length must be <= 256');
        }
    ...
    public function setAmount(string $amount) {
        if ((mb_strlen($amount) > 32)) {
            throw new IAE('TransferMultiToken.setAmount: $amount length must be <= 32');
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('TransferMultiToken.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('TransferMultiToken.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('TransferMultiToken.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('TransferMultiToken.setFromPrivateKey: $from_private_key length must be >= 66');
        }
