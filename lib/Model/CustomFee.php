...
class CustomFee extends AbstractModel {
    ...
    public function setGasLimit(string $gas_limit) {
        if ((!preg_match("/^[+]?\\d+$/", $gas_limit))) {
            throw new IAE('CustomFee.setGasLimit: $gas_limit must match /^[+]?\\d+$/, ' . var_export($gas_limit, true) . ' given');
        }
    ...
    public function setGasPrice(string $gas_price) {
        if ((!preg_match("/^[+]?\\d+$/", $gas_price))) {
            throw new IAE('CustomFee.setGasPrice: $gas_price must match /^[+]?\\d+$/, ' . var_export($gas_price, true) . ' given');
        }
