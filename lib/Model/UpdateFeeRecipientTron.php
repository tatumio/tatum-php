...
class UpdateFeeRecipientTron extends AbstractModel {
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "contract_address"=>null, "fee_recipient"=>null, "from_private_key"=>null, "fee_limit"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("UpdateFeeRecipientTron.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 34)) {
            throw new IAE('UpdateFeeRecipientTron.setContractAddress: $contract_address length must be <= 34');
        }
        if ((mb_strlen($contract_address) < 34)) {
            throw new IAE('UpdateFeeRecipientTron.setContractAddress: $contract_address length must be >= 34');
        }
    ...
    public function setFeeRecipient(string $fee_recipient) {
        if ((mb_strlen($fee_recipient) > 34)) {
            throw new IAE('UpdateFeeRecipientTron.setFeeRecipient: $fee_recipient length must be <= 34');
        }
        if ((mb_strlen($fee_recipient) < 34)) {
            throw new IAE('UpdateFeeRecipientTron.setFeeRecipient: $fee_recipient length must be >= 34');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 64)) {
            throw new IAE('UpdateFeeRecipientTron.setFromPrivateKey: $from_private_key length must be <= 64');
        }
        if ((mb_strlen($from_private_key) < 64)) {
            throw new IAE('UpdateFeeRecipientTron.setFromPrivateKey: $from_private_key length must be >= 64');
        }
    ...
    public function setFeeLimit(float $fee_limit) {
        if (($fee_limit < 0)) {
            throw new IAE('UpdateFeeRecipientTron.setFeeLimit: $fee_limit must be >=0');
        }
