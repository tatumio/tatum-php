...
/**
 * DeployKCSErc20OffchainKMSAddress Model
 *
 * @version   3.17.1
 ...
 * DeployKCSErc20OffchainKMSAddress Model
 */
class DeployKCSErc20OffchainKMSAddress extends AbstractModel {
    ...
    protected static $_name = "DeployKCSErc20OffchainKMSAddress";
    ...
    public function __construct(array $data = []) {
        foreach(["symbol"=>null, "supply"=>null, "description"=>null, "base_pair"=>null, "base_rate"=>1, "customer"=>null, "address"=>null, "signature_id"=>null, "nonce"=>null] as $k => $v) {
    ...
    public function setSymbol(string $symbol) {
        if ((mb_strlen($symbol) > 30)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setSymbol: $symbol length must be <= 30');
        }
        if ((mb_strlen($symbol) < 1)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setSymbol: $symbol length must be >= 1');
        }
        if ((!preg_match("/^[a-zA-Z0-9_]+$/", $symbol))) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setSymbol: $symbol must match /^[a-zA-Z0-9_]+$/, ' . var_export($symbol, true) . ' given');
        }
    ...
    public function setSupply(string $supply) {
        if ((mb_strlen($supply) > 38)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setSupply: $supply length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $supply))) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setSupply: $supply must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($supply, true) . ' given');
        }
    ...
    public function setDescription(string $description) {
        if ((mb_strlen($description) > 100)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setDescription: $description length must be <= 100');
        }
        if ((mb_strlen($description) < 1)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setDescription: $description length must be >= 1');
        }
    ...
    public function setBasePair(string $base_pair) {
        ...
        if (!in_array($base_pair, $allowed, true)) {
            throw new IAE(sprintf("DeployKCSErc20OffchainKMSAddress.setBasePair: base_pair invalid value '%s', must be one of '%s'", $base_pair, implode("', '", $allowed)));
        }
        if ((mb_strlen($base_pair) > 30)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setBasePair: $base_pair length must be <= 30');
        }
        if ((mb_strlen($base_pair) < 2)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setBasePair: $base_pair length must be >= 2');
        }
    ...
    public function setBaseRate(?float $base_rate) {
        if (!is_null($base_rate) && ($base_rate < 0)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setBaseRate: $base_rate must be >=0');
        }
    ...
    public function setAddress(string $address) {
        if ((mb_strlen($address) > 42)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setAddress: $address length must be <= 42');
        }
        if ((mb_strlen($address) < 42)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setAddress: $address length must be >= 42');
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('DeployKCSErc20OffchainKMSAddress.setNonce: $nonce must be >=0');
        }
