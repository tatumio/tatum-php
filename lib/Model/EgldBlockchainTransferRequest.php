...
/**
 * EgldBlockchainTransfer_request Model
 *
 * @version   3.17.0
 ...
 * EgldBlockchainTransfer_request Model
 */
class EgldBlockchainTransferRequest extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "EgldBlockchainTransfer_request";
    ...
    public function __construct(array $data = []) {
        foreach(["from"=>null, "to"=>null, "amount"=>null, "fee"=>null, "data"=>null, "from_private_key"=>null, "index"=>null, "signature_id"=>null] as $k => $v) {
    ...
    public function setFrom(string $from) {
        if ((mb_strlen($from) > 62)) {
            throw new IAE('EgldBlockchainTransferRequest.setFrom: $from length must be <= 62');
        }
        if ((mb_strlen($from) < 62)) {
            throw new IAE('EgldBlockchainTransferRequest.setFrom: $from length must be >= 62');
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 62)) {
            throw new IAE('EgldBlockchainTransferRequest.setTo: $to length must be <= 62');
        }
        if ((mb_strlen($to) < 62)) {
            throw new IAE('EgldBlockchainTransferRequest.setTo: $to length must be >= 62');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 64)) {
            throw new IAE('EgldBlockchainTransferRequest.setFromPrivateKey: $from_private_key length must be <= 64');
        }
        if ((mb_strlen($from_private_key) < 64)) {
            throw new IAE('EgldBlockchainTransferRequest.setFromPrivateKey: $from_private_key length must be >= 64');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('EgldBlockchainTransferRequest.setIndex: $index must be >=0');
        }
