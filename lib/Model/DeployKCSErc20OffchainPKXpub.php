...
/**
 * DeployKCSErc20OffchainPKXpub Model
 *
 * @version   3.17.0
 ...
 * DeployKCSErc20OffchainPKXpub Model
 */
class DeployKCSErc20OffchainPKXpub extends AbstractModel {
    ...
    protected static $_name = "DeployKCSErc20OffchainPKXpub";
    ...
    public function __construct(array $data = []) {
        foreach(["symbol"=>null, "supply"=>null, "description"=>null, "base_pair"=>null, "base_rate"=>1, "customer"=>null, "xpub"=>null, "derivation_index"=>null, "private_key"=>null, "nonce"=>null] as $k => $v) {
    ...
    public function setSymbol(string $symbol) {
        if ((mb_strlen($symbol) > 30)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setSymbol: $symbol length must be <= 30');
        }
        if ((mb_strlen($symbol) < 1)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setSymbol: $symbol length must be >= 1');
        }
        if ((!preg_match("/^[a-zA-Z0-9_]+$/", $symbol))) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setSymbol: $symbol must match /^[a-zA-Z0-9_]+$/, ' . var_export($symbol, true) . ' given');
        }
    ...
    public function setSupply(string $supply) {
        if ((mb_strlen($supply) > 38)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setSupply: $supply length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $supply))) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setSupply: $supply must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($supply, true) . ' given');
        }
    ...
    public function setDescription(string $description) {
        if ((mb_strlen($description) > 100)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setDescription: $description length must be <= 100');
        }
        if ((mb_strlen($description) < 1)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setDescription: $description length must be >= 1');
        }
    ...
    public function setBasePair(string $base_pair) {
        ...
        if (!in_array($base_pair, $allowed, true)) {
            throw new IAE(sprintf("DeployKCSErc20OffchainPKXpub.setBasePair: base_pair invalid value '%s', must be one of '%s'", $base_pair, implode("', '", $allowed)));
        }
        if ((mb_strlen($base_pair) > 30)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setBasePair: $base_pair length must be <= 30');
        }
        if ((mb_strlen($base_pair) < 2)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setBasePair: $base_pair length must be >= 2');
        }
    ...
    public function setBaseRate(?float $base_rate) {
        if (!is_null($base_rate) && ($base_rate < 0)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setBaseRate: $base_rate must be >=0');
        }
    ...
    public function setXpub(string $xpub) {
        if ((mb_strlen($xpub) > 150)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setXpub: $xpub length must be <= 150');
        }
        if ((mb_strlen($xpub) < 1)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setXpub: $xpub length must be >= 1');
        }
    ...
    public function setDerivationIndex(int $derivation_index) {
        if (($derivation_index > 2147483647)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setDerivationIndex: $derivation_index must be <=2147483647');
        }
    ...
    public function setPrivateKey(string $private_key) {
        if ((mb_strlen($private_key) > 66)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setPrivateKey: $private_key length must be <= 66');
        }
        if ((mb_strlen($private_key) < 66)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setPrivateKey: $private_key length must be >= 66');
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('DeployKCSErc20OffchainPKXpub.setNonce: $nonce must be >=0');
        }
