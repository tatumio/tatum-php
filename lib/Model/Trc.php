 ...
 * Trc Model
 */
class Trc extends AbstractModel {
    ...
    public function __construct(array $data = []) {
        foreach(["symbol"=>null, "supply"=>null, "decimals"=>null, "type"=>null, "description"=>null, "url"=>null, "base_pair"=>null, "base_rate"=>1, "customer"=>null, "accounting_currency"=>null, "derivation_index"=>null, "xpub"=>null, "address"=>null] as $k => $v) {
    ...
    public function listInvalidProperties(): array {
        ...
        if (!is_null($this->_data['derivation_index']) && ($this->_data['derivation_index'] > 2147483647)) {
            $ip[] = "'derivation_index' must be <= 2147483647";
        }
        if (!is_null($this->_data['xpub']) && (mb_strlen($this->_data['xpub']) > 130)) {
            $ip[] = "'xpub' length must be <= 130";
        }
        if (!is_null($this->_data['xpub']) && (mb_strlen($this->_data['xpub']) < 130)) {
            $ip[] = "'xpub' length must be >= 130";
        }
        if (!is_null($this->_data['address']) && (mb_strlen($this->_data['address']) > 34)) {
     ...
     * Set symbol
     * 
     * @param string $symbol TRC token name. Used as a identifier within Tatum system and also in blockchain as a currency symbol.
     * @return $this
     */
    public function setSymbol(string $symbol) {
        if ((mb_strlen($symbol) > 30)) {
            throw new IAE('Trc.setSymbol: $symbol length must be <= 30');
        }
        if ((mb_strlen($symbol) < 1)) {
            throw new IAE('Trc.setSymbol: $symbol length must be >= 1');
        }
        if ((!preg_match("/^[a-zA-Z0-9_]+$/", $symbol))) {
            throw new IAE('Trc.setSymbol: $symbol must match /^[a-zA-Z0-9_]+$/, ' . var_export($symbol, true) . ' given');
        }
     ...
     * Set supply
     * 
     * @param string $supply Supply of TRC tokens.
     * @return $this
     */
    public function setSupply(string $supply) {
        if ((mb_strlen($supply) > 38)) {
            throw new IAE('Trc.setSupply: $supply length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $supply))) {
            throw new IAE('Trc.setSupply: $supply must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($supply, true) . ' given');
        }
    ...
    public function setDecimals(float $decimals) {
        if (($decimals < 0)) {
            throw new IAE('Trc.setDecimals: $decimals must be >=0');
        }
    ...
    public function setType(string $type) {
        ...
        if (!in_array($type, $allowed, true)) {
            throw new IAE(sprintf("Trc.setType: type invalid value '%s', must be one of '%s'", $type, implode("', '", $allowed)));
        }
    ...
    public function setDescription(string $description) {
        if ((mb_strlen($description) > 100)) {
            throw new IAE('Trc.setDescription: $description length must be <= 100');
        }
        if ((mb_strlen($description) < 1)) {
            throw new IAE('Trc.setDescription: $description length must be >= 1');
        }
    ...
    public function setUrl(?string $url) {
        if (!is_null($url) && (mb_strlen($url) > 100)) {
            throw new IAE('Trc.setUrl: $url length must be <= 100');
        }
        if (!is_null($url) && (mb_strlen($url) < 1)) {
            throw new IAE('Trc.setUrl: $url length must be >= 1');
        }
        $this->_data['url'] = $url;
     ...
     * Set base_pair
     * 
     * @param string $base_pair Base pair for TRC token. Transaction value will be calculated according to this base pair.
     * @return $this
     */
    public function setBasePair(string $base_pair) {
        $allowed = $this->getBasePairAllowableValues();
        if (!in_array($base_pair, $allowed, true)) {
            throw new IAE(sprintf("Trc.setBasePair: base_pair invalid value '%s', must be one of '%s'", $base_pair, implode("', '", $allowed)));
        }
        if ((mb_strlen($base_pair) > 50)) {
            throw new IAE('Trc.setBasePair: $base_pair length must be <= 50');
        }
        if ((mb_strlen($base_pair) < 3)) {
            throw new IAE('Trc.setBasePair: $base_pair length must be >= 3');
        }
    ...
    public function setBaseRate(?float $base_rate) {
        if (!is_null($base_rate) && ($base_rate < 0)) {
            throw new IAE('Trc.setBaseRate: $base_rate must be >=0');
        }
    ...
    public function setAccountingCurrency(?string $accounting_currency) {
        ...
        if (!is_null($accounting_currency) && !in_array($accounting_currency, $allowed, true)) {
            throw new IAE(sprintf("Trc.setAccountingCurrency: accounting_currency invalid value '%s', must be one of '%s'", $accounting_currency, implode("', '", $allowed)));
        }
        if (!is_null($accounting_currency) && (mb_strlen($accounting_currency) > 3)) {
            throw new IAE('Trc.setAccountingCurrency: $accounting_currency length must be <= 3');
        }
        if (!is_null($accounting_currency) && (mb_strlen($accounting_currency) < 3)) {
            throw new IAE('Trc.setAccountingCurrency: $accounting_currency length must be >= 3');
        }
     ...
     * 
     * @param int|null $derivation_index Derivation index for xpub to generate specific deposit address.
     * @return $this
     */
    public function setDerivationIndex(?int $derivation_index) {
        if (!is_null($derivation_index) && ($derivation_index > 2147483647)) {
            throw new IAE('Trc.setDerivationIndex: $derivation_index must be <=2147483647');
        }
     ...
     * Set xpub
     * 
     * @param string|null $xpub Extended public key (xpub), from which address, where all initial supply will be stored, will be generated. Either xpub and derivationIndex, or address must be present, not both.
     * @return $this
     */
    public function setXpub(?string $xpub) {
        if (!is_null($xpub) && (mb_strlen($xpub) > 130)) {
            throw new IAE('Trc.setXpub: $xpub length must be <= 130');
        }
        if (!is_null($xpub) && (mb_strlen($xpub) < 130)) {
            throw new IAE('Trc.setXpub: $xpub length must be >= 130');
        }
     ...
     * Set address
     * 
     * @param string|null $address Address on Tron blockchain, where all initial supply will be stored. Either xpub and derivationIndex, or address must be present, not both.
     * @return $this
     */
    public function setAddress(?string $address) {
        if (!is_null($address) && (mb_strlen($address) > 34)) {
            throw new IAE('Trc.setAddress: $address length must be <= 34');
        }
        if (!is_null($address) && (mb_strlen($address) < 34)) {
            throw new IAE('Trc.setAddress: $address length must be >= 34');
        }
