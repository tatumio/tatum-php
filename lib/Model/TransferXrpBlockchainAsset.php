...
class TransferXrpBlockchainAsset extends AbstractModel {
    ...
    public function __construct(array $data = []) {
        foreach(["from_account"=>null, "to"=>null, "amount"=>null, "from_secret"=>null, "fee"=>null, "source_tag"=>null, "destination_tag"=>null, "issuer_account"=>null, "token"=>null] as $k => $v) {
    ...
    public function setFromAccount(string $from_account) {
        if ((mb_strlen($from_account) > 34)) {
            throw new IAE('TransferXrpBlockchainAsset.setFromAccount: $from_account length must be <= 34');
        }
        if ((mb_strlen($from_account) < 33)) {
            throw new IAE('TransferXrpBlockchainAsset.setFromAccount: $from_account length must be >= 33');
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 34)) {
            throw new IAE('TransferXrpBlockchainAsset.setTo: $to length must be <= 34');
        }
        if ((mb_strlen($to) < 33)) {
            throw new IAE('TransferXrpBlockchainAsset.setTo: $to length must be >= 33');
        }
        $this->_data['to'] = $to;
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('TransferXrpBlockchainAsset.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFromSecret(string $from_secret) {
        if ((mb_strlen($from_secret) > 29)) {
            throw new IAE('TransferXrpBlockchainAsset.setFromSecret: $from_secret length must be <= 29');
        }
        if ((mb_strlen($from_secret) < 29)) {
            throw new IAE('TransferXrpBlockchainAsset.setFromSecret: $from_secret length must be >= 29');
        }
    ...
    public function setFee(?string $fee) {
        if (!is_null($fee) && (!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $fee))) {
            throw new IAE('TransferXrpBlockchainAsset.setFee: $fee must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($fee, true) . ' given');
        }
        $this->_data['fee'] = $fee;
    ...
    public function setIssuerAccount(string $issuer_account) {
        if ((mb_strlen($issuer_account) > 34)) {
            throw new IAE('TransferXrpBlockchainAsset.setIssuerAccount: $issuer_account length must be <= 34');
        }
        if ((mb_strlen($issuer_account) < 33)) {
            throw new IAE('TransferXrpBlockchainAsset.setIssuerAccount: $issuer_account length must be >= 33');
        }
    ...
    public function setToken(string $token) {
        if ((mb_strlen($token) > 40)) {
            throw new IAE('TransferXrpBlockchainAsset.setToken: $token length must be <= 40');
        }
        if ((mb_strlen($token) < 40)) {
            throw new IAE('TransferXrpBlockchainAsset.setToken: $token length must be >= 40');
        }
        if ((!preg_match("/^[A-F0-9]{40}$/", $token))) {
            throw new IAE('TransferXrpBlockchainAsset.setToken: $token must match /^[A-F0-9]{40}$/, ' . var_export($token, true) . ' given');
        }
