...
/**
 * EthBlockchainSmartContractInvocation_request Model
 *
 ...
 * EthBlockchainSmartContractInvocation_request Model
 */
class EthBlockchainSmartContractInvocationRequest extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "EthBlockchainSmartContractInvocation_request";
    ...
    public function __construct(array $data = []) {
        foreach(["contract_address"=>null, "method_name"=>null, "method_abi"=>null, "params"=>null, "amount"=>null, "from_private_key"=>null, "nonce"=>null, "fee"=>null, "signature_id"=>null, "index"=>null] as $k => $v) {
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('EthBlockchainSmartContractInvocationRequest.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('EthBlockchainSmartContractInvocationRequest.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setMethodName(string $method_name) {
        if ((mb_strlen($method_name) > 500)) {
            throw new IAE('EthBlockchainSmartContractInvocationRequest.setMethodName: $method_name length must be <= 500');
        }
        if ((mb_strlen($method_name) < 1)) {
            throw new IAE('EthBlockchainSmartContractInvocationRequest.setMethodName: $method_name length must be >= 1');
        }
    ...
    public function setAmount(?string $amount) {
        if (!is_null($amount) && (!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('EthBlockchainSmartContractInvocationRequest.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('EthBlockchainSmartContractInvocationRequest.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('EthBlockchainSmartContractInvocationRequest.setFromPrivateKey: $from_private_key length must be >= 66');
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('EthBlockchainSmartContractInvocationRequest.setNonce: $nonce must be >=0');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('EthBlockchainSmartContractInvocationRequest.setIndex: $index must be >=0');
        }
