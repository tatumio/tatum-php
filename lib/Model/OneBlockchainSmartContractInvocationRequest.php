...
/**
 * OneBlockchainSmartContractInvocation_request Model
 *
 * @version   3.17.0
 ...
 * OneBlockchainSmartContractInvocation_request Model
 */
class OneBlockchainSmartContractInvocationRequest extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "OneBlockchainSmartContractInvocation_request";
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('OneBlockchainSmartContractInvocationRequest.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('OneBlockchainSmartContractInvocationRequest.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setMethodName(string $method_name) {
        if ((mb_strlen($method_name) > 500)) {
            throw new IAE('OneBlockchainSmartContractInvocationRequest.setMethodName: $method_name length must be <= 500');
        }
        if ((mb_strlen($method_name) < 1)) {
            throw new IAE('OneBlockchainSmartContractInvocationRequest.setMethodName: $method_name length must be >= 1');
        }
    ...
    public function setAmount(?string $amount) {
        if (!is_null($amount) && (!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('OneBlockchainSmartContractInvocationRequest.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('OneBlockchainSmartContractInvocationRequest.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('OneBlockchainSmartContractInvocationRequest.setFromPrivateKey: $from_private_key length must be >= 66');
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('OneBlockchainSmartContractInvocationRequest.setNonce: $nonce must be >=0');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('OneBlockchainSmartContractInvocationRequest.setIndex: $index must be >=0');
        }
