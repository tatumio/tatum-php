 ...
 * GenerateAuction_request Model
 */
class GenerateAuctionRequest extends AbstractModel {
    ...
    protected static $_name = "GenerateAuction_request";
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "fee_recipient"=>null, "auction_fee"=>null, "from_private_key"=>null, "nonce"=>null, "fee"=>null, "signature_id"=>null, "index"=>null, "fee_currency"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("GenerateAuctionRequest.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setFeeRecipient(string $fee_recipient) {
        if ((mb_strlen($fee_recipient) > 42)) {
            throw new IAE('GenerateAuctionRequest.setFeeRecipient: $fee_recipient length must be <= 42');
        }
        if ((mb_strlen($fee_recipient) < 42)) {
            throw new IAE('GenerateAuctionRequest.setFeeRecipient: $fee_recipient length must be >= 42');
        }
    ...
    public function setAuctionFee(float $auction_fee) {
        if (($auction_fee > 10000)) {
            throw new IAE('GenerateAuctionRequest.setAuctionFee: $auction_fee must be <=10000');
        }
        if (($auction_fee < 0)) {
            throw new IAE('GenerateAuctionRequest.setAuctionFee: $auction_fee must be >=0');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('GenerateAuctionRequest.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('GenerateAuctionRequest.setFromPrivateKey: $from_private_key length must be >= 66');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('GenerateAuctionRequest.setIndex: $index must be >=0');
        }
    ...
    public function setFeeCurrency(string $fee_currency) {
        ...
        if (!in_array($fee_currency, $allowed, true)) {
            throw new IAE(sprintf("GenerateAuctionRequest.setFeeCurrency: fee_currency invalid value '%s', must be one of '%s'", $fee_currency, implode("', '", $allowed)));
        }
