...
/**
 * KlaytnBlockchainTransfer_request Model
 *
 * @version   3.17.0
 ...
 * KlaytnBlockchainTransfer_request Model
 */
class KlaytnBlockchainTransferRequest extends AbstractModel {

    public const DISCRIMINATOR = null;
    public const CURRENCY_KLAY = 'KLAY';
    protected static $_name = "KlaytnBlockchainTransfer_request";
     ...
     * Set data
     * 
     * @param string|null $data Additional data, that can be passed to blockchain transaction as data property. Only for KLAY transactions.
     * @return $this
     */
    public function setData(?string $data) {
        if (!is_null($data) && (mb_strlen($data) > 50000)) {
            throw new IAE('KlaytnBlockchainTransferRequest.setData: $data length must be <= 50000');
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('KlaytnBlockchainTransferRequest.setNonce: $nonce must be >=0');
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 42)) {
            throw new IAE('KlaytnBlockchainTransferRequest.setTo: $to length must be <= 42');
        }
        if ((mb_strlen($to) < 42)) {
            throw new IAE('KlaytnBlockchainTransferRequest.setTo: $to length must be >= 42');
        }
    ...
    public function setCurrency(string $currency) {
        ...
        if (!in_array($currency, $allowed, true)) {
            throw new IAE(sprintf("KlaytnBlockchainTransferRequest.setCurrency: currency invalid value '%s', must be one of '%s'", $currency, implode("', '", $allowed)));
        }
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('KlaytnBlockchainTransferRequest.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('KlaytnBlockchainTransferRequest.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('KlaytnBlockchainTransferRequest.setFromPrivateKey: $from_private_key length must be >= 66');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('KlaytnBlockchainTransferRequest.setIndex: $index must be >=0');
        }
