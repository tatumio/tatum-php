...
/**
 * TrustLineXrpBlockchainKMS Model
 *
 ...
 * TrustLineXrpBlockchainKMS Model
 */
class TrustLineXrpBlockchainKMS extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "TrustLineXrpBlockchainKMS";
    ...
    public function __construct(array $data = []) {
        foreach(["from_account"=>null, "issuer_account"=>null, "limit"=>null, "token"=>null, "signature_id"=>null, "fee"=>null] as $k => $v) {
    ...
    public function setFromAccount(string $from_account) {
        if ((mb_strlen($from_account) > 34)) {
            throw new IAE('TrustLineXrpBlockchainKMS.setFromAccount: $from_account length must be <= 34');
        }
        if ((mb_strlen($from_account) < 33)) {
            throw new IAE('TrustLineXrpBlockchainKMS.setFromAccount: $from_account length must be >= 33');
        }
    ...
    public function setIssuerAccount(string $issuer_account) {
        if ((mb_strlen($issuer_account) > 34)) {
            throw new IAE('TrustLineXrpBlockchainKMS.setIssuerAccount: $issuer_account length must be <= 34');
        }
        if ((mb_strlen($issuer_account) < 33)) {
            throw new IAE('TrustLineXrpBlockchainKMS.setIssuerAccount: $issuer_account length must be >= 33');
        }
    ...
    public function setLimit(string $limit) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $limit))) {
            throw new IAE('TrustLineXrpBlockchainKMS.setLimit: $limit must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($limit, true) . ' given');
        }
    ...
    public function setToken(string $token) {
        if ((mb_strlen($token) > 40)) {
            throw new IAE('TrustLineXrpBlockchainKMS.setToken: $token length must be <= 40');
        }
        if ((mb_strlen($token) < 40)) {
            throw new IAE('TrustLineXrpBlockchainKMS.setToken: $token length must be >= 40');
        }
        if ((!preg_match("/^[A-F0-9]{40}$/", $token))) {
            throw new IAE('TrustLineXrpBlockchainKMS.setToken: $token must match /^[A-F0-9]{40}$/, ' . var_export($token, true) . ' given');
        }
    ...
    public function setFee(?string $fee) {
        if (!is_null($fee) && (!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $fee))) {
            throw new IAE('TrustLineXrpBlockchainKMS.setFee: $fee must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($fee, true) . ' given');
        }
        $this->_data['fee'] = $fee;
