 ...
 * AlgoTransfer_request Model
 */
class AlgoTransferRequest extends AbstractModel {
    ...
    public function setAddress(string $address) {
        if ((mb_strlen($address) > 58)) {
            throw new IAE('AlgoTransferRequest.setAddress: $address length must be <= 58');
        }
        if ((mb_strlen($address) < 58)) {
            throw new IAE('AlgoTransferRequest.setAddress: $address length must be >= 58');
        }
    ...
    public function setAmount(string $amount) {
        if ((mb_strlen($amount) > 38)) {
            throw new IAE('AlgoTransferRequest.setAmount: $amount length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('AlgoTransferRequest.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setPrivateKey(string $private_key) {
        if ((mb_strlen($private_key) > 103)) {
            throw new IAE('AlgoTransferRequest.setPrivateKey: $private_key length must be <= 103');
        }
        if ((mb_strlen($private_key) < 103)) {
            throw new IAE('AlgoTransferRequest.setPrivateKey: $private_key length must be >= 103');
        }
    ...
    public function setPaymentId(?string $payment_id) {
        if (!is_null($payment_id) && (mb_strlen($payment_id) > 100)) {
            throw new IAE('AlgoTransferRequest.setPaymentId: $payment_id length must be <= 100');
        }
        if (!is_null($payment_id) && (mb_strlen($payment_id) < 1)) {
            throw new IAE('AlgoTransferRequest.setPaymentId: $payment_id length must be >= 1');
        }
    ...
    public function setSenderNote(?string $sender_note) {
        if (!is_null($sender_note) && (mb_strlen($sender_note) > 500)) {
            throw new IAE('AlgoTransferRequest.setSenderNote: $sender_note length must be <= 500');
        }
        if (!is_null($sender_note) && (mb_strlen($sender_note) < 1)) {
            throw new IAE('AlgoTransferRequest.setSenderNote: $sender_note length must be >= 1');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('AlgoTransferRequest.setIndex: $index must be >=0');
        }
