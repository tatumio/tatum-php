...
/**
 * TransferBchKeyPair_keyPair_inner Model
 *
 * @version   3.17.0
 ...
 * TransferBchKeyPair_keyPair_inner Model
 */
class TransferBchKeyPairKeyPairInner extends AbstractModel {

    public const DISCRIMINATOR = null;
    protected static $_name = "TransferBchKeyPair_keyPair_inner";
    ...
    public function setAddress(?string $address) {
        if (!is_null($address) && (mb_strlen($address) > 50)) {
            throw new IAE('TransferBchKeyPairKeyPairInner.setAddress: $address length must be <= 50');
        }
        if (!is_null($address) && (mb_strlen($address) < 30)) {
            throw new IAE('TransferBchKeyPairKeyPairInner.setAddress: $address length must be >= 30');
        }
    ...
    public function setPrivateKey(?string $private_key) {
        if (!is_null($private_key) && (mb_strlen($private_key) > 52)) {
            throw new IAE('TransferBchKeyPairKeyPairInner.setPrivateKey: $private_key length must be <= 52');
        }
        if (!is_null($private_key) && (mb_strlen($private_key) < 52)) {
            throw new IAE('TransferBchKeyPairKeyPairInner.setPrivateKey: $private_key length must be >= 52');
        }
