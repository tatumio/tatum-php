...
/**
 * ListOderBookActiveSellBody Model
 *
 * @version   3.17.0
 ...
 * ListOderBookActiveSellBody Model
 */
class ListOderBookActiveSellBody extends AbstractModel {

    public const DISCRIMINATOR = null;
    public const TRADE_TYPE_FUTURE_SELL = 'FUTURE_SELL';
    ...
    public function setPageSize(float $page_size) {
        if (($page_size > 50)) {
            throw new IAE('ListOderBookActiveSellBody.setPageSize: $page_size must be <=50');
        }
        if (($page_size < 1)) {
            throw new IAE('ListOderBookActiveSellBody.setPageSize: $page_size must be >=1');
        }
    ...
    public function setPair(?string $pair) {
        if (!is_null($pair) && (mb_strlen($pair) > 30)) {
            throw new IAE('ListOderBookActiveSellBody.setPair: $pair length must be <= 30');
        }
        if (!is_null($pair) && (mb_strlen($pair) < 3)) {
            throw new IAE('ListOderBookActiveSellBody.setPair: $pair length must be >= 3');
        }
        if (!is_null($pair) && (!preg_match("/^[A-a-zZ0-9_\\-]+\/[A-Za-z0-9_\\-]+$/", $pair))) {
            throw new IAE('ListOderBookActiveSellBody.setPair: $pair must match /^[A-a-zZ0-9_\\-]+\/[A-Za-z0-9_\\-]+$/, ' . var_export($pair, true) . ' given');
        }
    ...
    public function setTradeType(?string $trade_type) {
        ...
        if (!is_null($trade_type) && !in_array($trade_type, $allowed, true)) {
            throw new IAE(sprintf("ListOderBookActiveSellBody.setTradeType: trade_type invalid value '%s', must be one of '%s'", $trade_type, implode("', '", $allowed)));
        }
    ...
    public function setSort(?array $sort) {
        ...
        if (!is_null($sort) && array_diff($sort, $allowed)) {
            throw new IAE(sprintf("ListOderBookActiveSellBody.setSort: sort must be one of '%s'", implode("', '", $allowed)));
        }
