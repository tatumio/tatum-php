...
class CreateFutureTrade extends AbstractModel {
    ...
    public function setType(string $type) {
        ...
        if (!in_array($type, $allowed, true)) {
            throw new IAE(sprintf("CreateFutureTrade.setType: type invalid value '%s', must be one of '%s'", $type, implode("', '", $allowed)));
        }
    ...
    public function setPrice(string $price) {
        if ((mb_strlen($price) > 38)) {
            throw new IAE('CreateFutureTrade.setPrice: $price length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $price))) {
            throw new IAE('CreateFutureTrade.setPrice: $price must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($price, true) . ' given');
        }
    ...
    public function setAmount(string $amount) {
        if ((mb_strlen($amount) > 38)) {
            throw new IAE('CreateFutureTrade.setAmount: $amount length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('CreateFutureTrade.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setPair(string $pair) {
        if ((mb_strlen($pair) > 30)) {
            throw new IAE('CreateFutureTrade.setPair: $pair length must be <= 30');
        }
        if ((mb_strlen($pair) < 3)) {
            throw new IAE('CreateFutureTrade.setPair: $pair length must be >= 3');
        }
        if ((!preg_match("/^[A-a-zZ0-9_\\-]+\/[A-Za-z0-9_\\-]+$/", $pair))) {
            throw new IAE('CreateFutureTrade.setPair: $pair must match /^[A-a-zZ0-9_\\-]+\/[A-Za-z0-9_\\-]+$/, ' . var_export($pair, true) . ' given');
        }
    ...
    public function setCurrency1AccountId(string $currency1_account_id) {
        if ((mb_strlen($currency1_account_id) > 24)) {
            throw new IAE('CreateFutureTrade.setCurrency1AccountId: $currency1_account_id length must be <= 24');
        }
        if ((mb_strlen($currency1_account_id) < 24)) {
            throw new IAE('CreateFutureTrade.setCurrency1AccountId: $currency1_account_id length must be >= 24');
        }
    ...
    public function setCurrency2AccountId(string $currency2_account_id) {
        if ((mb_strlen($currency2_account_id) > 24)) {
            throw new IAE('CreateFutureTrade.setCurrency2AccountId: $currency2_account_id length must be <= 24');
        }
        if ((mb_strlen($currency2_account_id) < 24)) {
            throw new IAE('CreateFutureTrade.setCurrency2AccountId: $currency2_account_id length must be >= 24');
        }
    ...
    public function setFeeAccountId(?string $fee_account_id) {
        if (!is_null($fee_account_id) && (mb_strlen($fee_account_id) > 24)) {
            throw new IAE('CreateFutureTrade.setFeeAccountId: $fee_account_id length must be <= 24');
        }
        if (!is_null($fee_account_id) && (mb_strlen($fee_account_id) < 24)) {
            throw new IAE('CreateFutureTrade.setFeeAccountId: $fee_account_id length must be >= 24');
        }
    ...
    public function setFee(?float $fee) {
        if (!is_null($fee) && ($fee > 100)) {
            throw new IAE('CreateFutureTrade.setFee: $fee must be <=100');
        }
        if (!is_null($fee) && ($fee < 0)) {
            throw new IAE('CreateFutureTrade.setFee: $fee must be >=0');
        }
        $this->_data['fee'] = $fee;
