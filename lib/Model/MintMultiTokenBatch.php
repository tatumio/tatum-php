 ...
 * MintMultiTokenBatch Model
 */
class MintMultiTokenBatch extends AbstractModel {
    ...
    protected static $_name = "MintMultiTokenBatch";
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("MintMultiTokenBatch.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('MintMultiTokenBatch.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('MintMultiTokenBatch.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('MintMultiTokenBatch.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('MintMultiTokenBatch.setFromPrivateKey: $from_private_key length must be >= 66');
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('MintMultiTokenBatch.setNonce: $nonce must be >=0');
        }
