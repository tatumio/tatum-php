...
class CreateAccount extends AbstractModel {
    ...
    public function __construct(array $data = []) {
        foreach(["currency"=>null, "customer"=>null, "compliant"=>null, "account_code"=>null, "accounting_currency"=>null, "account_number"=>null] as $k => $v) {
    ...
    public function setCurrency(string $currency) {
        if ((mb_strlen($currency) > 40)) {
            throw new IAE('CreateAccount.setCurrency: $currency length must be <= 40');
        }
        if ((mb_strlen($currency) < 2)) {
            throw new IAE('CreateAccount.setCurrency: $currency length must be >= 2');
        }
    ...
    public function setAccountCode(?string $account_code) {
        if (!is_null($account_code) && (mb_strlen($account_code) > 50)) {
            throw new IAE('CreateAccount.setAccountCode: $account_code length must be <= 50');
        }
        if (!is_null($account_code) && (mb_strlen($account_code) < 1)) {
            throw new IAE('CreateAccount.setAccountCode: $account_code length must be >= 1');
        }
    ...
    public function setAccountingCurrency(?string $accounting_currency) {
        ...
        if (!is_null($accounting_currency) && !in_array($accounting_currency, $allowed, true)) {
            throw new IAE(sprintf("CreateAccount.setAccountingCurrency: accounting_currency invalid value '%s', must be one of '%s'", $accounting_currency, implode("', '", $allowed)));
        }
        if (!is_null($accounting_currency) && (mb_strlen($accounting_currency) > 3)) {
            throw new IAE('CreateAccount.setAccountingCurrency: $accounting_currency length must be <= 3');
        }
        if (!is_null($accounting_currency) && (mb_strlen($accounting_currency) < 3)) {
            throw new IAE('CreateAccount.setAccountingCurrency: $accounting_currency length must be >= 3');
        }
    ...
    public function setAccountNumber(?string $account_number) {
        if (!is_null($account_number) && (mb_strlen($account_number) > 50)) {
            throw new IAE('CreateAccount.setAccountNumber: $account_number length must be <= 50');
        }
        if (!is_null($account_number) && (mb_strlen($account_number) < 1)) {
            throw new IAE('CreateAccount.setAccountNumber: $account_number length must be >= 1');
        }
