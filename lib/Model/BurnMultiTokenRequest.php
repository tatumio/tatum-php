 ...
 * BurnMultiToken_request Model
 */
class BurnMultiTokenRequest extends AbstractModel {
    ...
    protected static $_name = "BurnMultiToken_request";
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "account"=>null, "token_id"=>null, "contract_address"=>null, "from_private_key"=>null, "data"=>null, "amount"=>null, "nonce"=>null, "fee"=>null, "index"=>null, "signature_id"=>null, "fee_currency"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("BurnMultiTokenRequest.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setAccount(string $account) {
        if ((mb_strlen($account) > 42)) {
            throw new IAE('BurnMultiTokenRequest.setAccount: $account length must be <= 42');
        }
        if ((mb_strlen($account) < 42)) {
            throw new IAE('BurnMultiTokenRequest.setAccount: $account length must be >= 42');
        }
    ...
    public function setTokenId(string $token_id) {
        if ((mb_strlen($token_id) > 32)) {
            throw new IAE('BurnMultiTokenRequest.setTokenId: $token_id length must be <= 32');
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('BurnMultiTokenRequest.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('BurnMultiTokenRequest.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('BurnMultiTokenRequest.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('BurnMultiTokenRequest.setFromPrivateKey: $from_private_key length must be >= 66');
        }
    ...
    public function setAmount(string $amount) {
        if ((mb_strlen($amount) > 32)) {
            throw new IAE('BurnMultiTokenRequest.setAmount: $amount length must be <= 32');
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('BurnMultiTokenRequest.setNonce: $nonce must be >=0');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('BurnMultiTokenRequest.setIndex: $index must be >=0');
        }
    ...
    public function setFeeCurrency(string $fee_currency) {
        ...
        if (!in_array($fee_currency, $allowed, true)) {
            throw new IAE(sprintf("BurnMultiTokenRequest.setFeeCurrency: fee_currency invalid value '%s', must be one of '%s'", $fee_currency, implode("', '", $allowed)));
        }
