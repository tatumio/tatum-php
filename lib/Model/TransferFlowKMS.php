...
class TransferFlowKMS extends AbstractModel {
    ...
    public function __construct(array $data = []) {
        foreach(["sender_account_id"=>null, "account"=>null, "address"=>null, "amount"=>null, "signature_id"=>null, "index"=>null, "compliant"=>null, "payment_id"=>null, "sender_note"=>null] as $k => $v) {
    ...
    public function setSenderAccountId(string $sender_account_id) {
        if ((mb_strlen($sender_account_id) > 24)) {
            throw new IAE('TransferFlowKMS.setSenderAccountId: $sender_account_id length must be <= 24');
        }
        if ((mb_strlen($sender_account_id) < 24)) {
            throw new IAE('TransferFlowKMS.setSenderAccountId: $sender_account_id length must be >= 24');
        }
    ...
    public function setAccount(string $account) {
        if ((mb_strlen($account) > 18)) {
            throw new IAE('TransferFlowKMS.setAccount: $account length must be <= 18');
        }
        if ((mb_strlen($account) < 18)) {
            throw new IAE('TransferFlowKMS.setAccount: $account length must be >= 18');
        }
    ...
    public function setAddress(string $address) {
        if ((mb_strlen($address) > 18)) {
            throw new IAE('TransferFlowKMS.setAddress: $address length must be <= 18');
        }
        if ((mb_strlen($address) < 18)) {
            throw new IAE('TransferFlowKMS.setAddress: $address length must be >= 18');
        }
    ...
    public function setAmount(string $amount) {
        if ((mb_strlen($amount) > 38)) {
            throw new IAE('TransferFlowKMS.setAmount: $amount length must be <= 38');
        }
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('TransferFlowKMS.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('TransferFlowKMS.setIndex: $index must be >=0');
        }
    ...
    public function setPaymentId(?string $payment_id) {
        if (!is_null($payment_id) && (mb_strlen($payment_id) > 100)) {
            throw new IAE('TransferFlowKMS.setPaymentId: $payment_id length must be <= 100');
        }
        if (!is_null($payment_id) && (mb_strlen($payment_id) < 1)) {
            throw new IAE('TransferFlowKMS.setPaymentId: $payment_id length must be >= 1');
        }
    ...
    public function setSenderNote(?string $sender_note) {
        if (!is_null($sender_note) && (mb_strlen($sender_note) > 500)) {
            throw new IAE('TransferFlowKMS.setSenderNote: $sender_note length must be <= 500');
        }
        if (!is_null($sender_note) && (mb_strlen($sender_note) < 1)) {
            throw new IAE('TransferFlowKMS.setSenderNote: $sender_note length must be >= 1');
        }
