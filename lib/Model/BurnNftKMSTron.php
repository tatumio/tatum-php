 ...
 * BurnNftKMSTron Model
 */
class BurnNftKMSTron extends AbstractModel {

    public const DISCRIMINATOR = null;
    public const CHAIN_TRON = 'TRON';
    protected static $_name = "BurnNftKMSTron";
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "account"=>null, "token_id"=>null, "contract_address"=>null, "index"=>null, "signature_id"=>null, "fee_limit"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("BurnNftKMSTron.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setAccount(string $account) {
        if ((mb_strlen($account) > 34)) {
            throw new IAE('BurnNftKMSTron.setAccount: $account length must be <= 34');
        }
        if ((mb_strlen($account) < 34)) {
            throw new IAE('BurnNftKMSTron.setAccount: $account length must be >= 34');
        }
    ...
    public function setTokenId(string $token_id) {
        if ((mb_strlen($token_id) > 78)) {
            throw new IAE('BurnNftKMSTron.setTokenId: $token_id length must be <= 78');
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 34)) {
            throw new IAE('BurnNftKMSTron.setContractAddress: $contract_address length must be <= 34');
        }
        if ((mb_strlen($contract_address) < 34)) {
            throw new IAE('BurnNftKMSTron.setContractAddress: $contract_address length must be >= 34');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('BurnNftKMSTron.setIndex: $index must be >=0');
        }
