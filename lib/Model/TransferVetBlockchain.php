 ...
 * TransferVetBlockchain Model
 */
class TransferVetBlockchain extends AbstractModel {
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 100)) {
            throw new IAE('TransferVetBlockchain.setTo: $to length must be <= 100');
        }
        if ((mb_strlen($to) < 1)) {
            throw new IAE('TransferVetBlockchain.setTo: $to length must be >= 1');
        }
        $this->_data['to'] = $to;
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('TransferVetBlockchain.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('TransferVetBlockchain.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 6)) {
            throw new IAE('TransferVetBlockchain.setFromPrivateKey: $from_private_key length must be >= 6');
        }
     ...
     * Set data
     * 
     * @param string|null $data Additinal data, that can be passed to blockchain transaction as data property.
     * @return $this
     */
    public function setData(?string $data) {
        if (!is_null($data) && (mb_strlen($data) > 10000)) {
            throw new IAE('TransferVetBlockchain.setData: $data length must be <= 10000');
        }
