...
/**
 * ApproveTransferCustodialWalletCelo Model
 *
 * @version   3.17.0
 ...
 * ApproveTransferCustodialWalletCelo Model
 */
class ApproveTransferCustodialWalletCelo extends AbstractModel {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("ApproveTransferCustodialWalletCelo.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setCustodialAddress(string $custodial_address) {
        if ((mb_strlen($custodial_address) > 42)) {
            throw new IAE('ApproveTransferCustodialWalletCelo.setCustodialAddress: $custodial_address length must be <= 42');
        }
        if ((mb_strlen($custodial_address) < 42)) {
            throw new IAE('ApproveTransferCustodialWalletCelo.setCustodialAddress: $custodial_address length must be >= 42');
        }
    ...
    public function setSpender(string $spender) {
        if ((mb_strlen($spender) > 42)) {
            throw new IAE('ApproveTransferCustodialWalletCelo.setSpender: $spender length must be <= 42');
        }
        if ((mb_strlen($spender) < 42)) {
            throw new IAE('ApproveTransferCustodialWalletCelo.setSpender: $spender length must be >= 42');
        }
    ...
    public function setContractType(float $contract_type) {
        ...
        if (!in_array($contract_type, $allowed, true)) {
            throw new IAE(sprintf("ApproveTransferCustodialWalletCelo.setContractType: contract_type invalid value '%s', must be one of '%s'", $contract_type, implode("', '", $allowed)));
        }
    ...
    public function setTokenAddress(string $token_address) {
        if ((mb_strlen($token_address) > 42)) {
            throw new IAE('ApproveTransferCustodialWalletCelo.setTokenAddress: $token_address length must be <= 42');
        }
        if ((mb_strlen($token_address) < 42)) {
            throw new IAE('ApproveTransferCustodialWalletCelo.setTokenAddress: $token_address length must be >= 42');
        }
    ...
    public function setAmount(?string $amount) {
        if (!is_null($amount) && (!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('ApproveTransferCustodialWalletCelo.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
    ...
    public function setTokenId(?string $token_id) {
        if (!is_null($token_id) && (mb_strlen($token_id) > 256)) {
            throw new IAE('ApproveTransferCustodialWalletCelo.setTokenId: $token_id length must be <= 256');
        }
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('ApproveTransferCustodialWalletCelo.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('ApproveTransferCustodialWalletCelo.setFromPrivateKey: $from_private_key length must be >= 66');
        }
    ...
    public function setFeeCurrency(string $fee_currency) {
        ...
        if (!in_array($fee_currency, $allowed, true)) {
            throw new IAE(sprintf("ApproveTransferCustodialWalletCelo.setFeeCurrency: fee_currency invalid value '%s', must be one of '%s'", $fee_currency, implode("', '", $allowed)));
        }
