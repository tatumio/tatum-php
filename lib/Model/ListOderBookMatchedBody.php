...
class ListOderBookMatchedBody extends AbstractModel {
    ...
    protected static $_name = "ListOderBookMatchedBody";
    ...
    public function __construct(array $data = []) {
        foreach(["id"=>null, "customer_id"=>null, "page_size"=>null, "offset"=>null, "pair"=>null, "count"=>null, "type"=>null, "amount"=>null, "fill"=>null, "price"=>null, "created"=>null, "sort"=>null] as $k => $v) {
     ...
     * Set id
     * 
     * @param string|null $id Account ID. If present, list matched FUTURE_BUY/FUTURE_SELL orders trades for that account.
     * @return $this
     */
     ...
     * Set customer_id
     * 
     * @param string|null $customer_id Customer ID. If present, list matched FUTURE_BUY/FUTURE_SELL orders trades for that customer.
     * @return $this
     */
    ...
    public function setPageSize(float $page_size) {
        if (($page_size > 50)) {
            throw new IAE('ListOderBookMatchedBody.setPageSize: $page_size must be <=50');
        }
        if (($page_size < 1)) {
            throw new IAE('ListOderBookMatchedBody.setPageSize: $page_size must be >=1');
        }
     ...
     * Set pair
     * 
     * @param string|null $pair Trade pair. If present, list matched FUTURE_BUY/FUTURE_SELL orders trades for that pair.
     * @return $this
     */
    public function setPair(?string $pair) {
        if (!is_null($pair) && (mb_strlen($pair) > 30)) {
            throw new IAE('ListOderBookMatchedBody.setPair: $pair length must be <= 30');
        }
        if (!is_null($pair) && (mb_strlen($pair) < 3)) {
            throw new IAE('ListOderBookMatchedBody.setPair: $pair length must be >= 3');
        }
        if (!is_null($pair) && (!preg_match("/^[A-a-zZ0-9_\\-]+\/[A-Za-z0-9_\\-]+$/", $pair))) {
            throw new IAE('ListOderBookMatchedBody.setPair: $pair must match /^[A-a-zZ0-9_\\-]+\/[A-Za-z0-9_\\-]+$/, ' . var_export($pair, true) . ' given');
        }
     ...
     * Set type
     * 
     * @param string|null $type Trade type.
     * @return $this
     */
    public function setType(?string $type) {
        $allowed = $this->getTypeAllowableValues();
        if (!is_null($type) && !in_array($type, $allowed, true)) {
            throw new IAE(sprintf("ListOderBookMatchedBody.setType: type invalid value '%s', must be one of '%s'", $type, implode("', '", $allowed)));
        }
    ...
    public function setSort(?array $sort) {
        ...
        if (!is_null($sort) && array_diff($sort, $allowed)) {
            throw new IAE(sprintf("ListOderBookMatchedBody.setSort: sort must be one of '%s'", implode("', '", $allowed)));
        }
