...
/**
 * BurnMultiTokenBatchKMSCelo Model
 *
 ...
 * BurnMultiTokenBatchKMSCelo Model
 */
class BurnMultiTokenBatchKMSCelo extends AbstractModel {
    ...
    protected static $_name = "BurnMultiTokenBatchKMSCelo";
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "account"=>null, "token_id"=>null, "amounts"=>null, "contract_address"=>null, "index"=>null, "signature_id"=>null, "nonce"=>null, "fee_currency"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("BurnMultiTokenBatchKMSCelo.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setAccount(string $account) {
        if ((mb_strlen($account) > 42)) {
            throw new IAE('BurnMultiTokenBatchKMSCelo.setAccount: $account length must be <= 42');
        }
        if ((mb_strlen($account) < 42)) {
            throw new IAE('BurnMultiTokenBatchKMSCelo.setAccount: $account length must be >= 42');
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('BurnMultiTokenBatchKMSCelo.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('BurnMultiTokenBatchKMSCelo.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setIndex(?float $index) {
        if (!is_null($index) && ($index < 0)) {
            throw new IAE('BurnMultiTokenBatchKMSCelo.setIndex: $index must be >=0');
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('BurnMultiTokenBatchKMSCelo.setNonce: $nonce must be >=0');
        }
    ...
    public function setFeeCurrency(string $fee_currency) {
        ...
        if (!in_array($fee_currency, $allowed, true)) {
            throw new IAE(sprintf("BurnMultiTokenBatchKMSCelo.setFeeCurrency: fee_currency invalid value '%s', must be one of '%s'", $fee_currency, implode("', '", $allowed)));
        }
