 ...
 * CancelOrSettleAuction Model
 */
class CancelOrSettleAuction extends AbstractModel {
    ...
    protected static $_name = "CancelOrSettleAuction";
    ...
    public function __construct(array $data = []) {
        foreach(["chain"=>null, "contract_address"=>null, "erc20_address"=>null, "id"=>null, "from_private_key"=>null, "nonce"=>null, "fee"=>null] as $k => $v) {
    ...
    public function setChain(string $chain) {
        ...
        if (!in_array($chain, $allowed, true)) {
            throw new IAE(sprintf("CancelOrSettleAuction.setChain: chain invalid value '%s', must be one of '%s'", $chain, implode("', '", $allowed)));
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('CancelOrSettleAuction.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('CancelOrSettleAuction.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setErc20Address(?string $erc20_address) {
        if (!is_null($erc20_address) && (mb_strlen($erc20_address) > 42)) {
            throw new IAE('CancelOrSettleAuction.setErc20Address: $erc20_address length must be <= 42');
        }
        if (!is_null($erc20_address) && (mb_strlen($erc20_address) < 42)) {
            throw new IAE('CancelOrSettleAuction.setErc20Address: $erc20_address length must be >= 42');
        }
    ...
    public function setId(string $id) {
        if ((mb_strlen($id) > 200)) {
            throw new IAE('CancelOrSettleAuction.setId: $id length must be <= 200');
        }
        if ((mb_strlen($id) < 1)) {
            throw new IAE('CancelOrSettleAuction.setId: $id length must be >= 1');
        }
        $this->_data['id'] = $id;
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('CancelOrSettleAuction.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('CancelOrSettleAuction.setFromPrivateKey: $from_private_key length must be >= 66');
        }
