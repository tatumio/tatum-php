...
class OneEstimateGas extends AbstractModel {
    ...
    public function listInvalidProperties(): array {
        ...
        if ((mb_strlen($this->_data['from']) > 66)) {
            $ip[] = "'from' length must be <= 66";
        }
        if ((mb_strlen($this->_data['from']) < 66)) {
            $ip[] = "'from' length must be >= 66";
        }
    ...
    public function setFrom(string $from) {
        if ((mb_strlen($from) > 66)) {
            throw new IAE('OneEstimateGas.setFrom: $from length must be <= 66');
        }
        if ((mb_strlen($from) < 66)) {
            throw new IAE('OneEstimateGas.setFrom: $from length must be >= 66');
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 42)) {
            throw new IAE('OneEstimateGas.setTo: $to length must be <= 42');
        }
        if ((mb_strlen($to) < 42)) {
            throw new IAE('OneEstimateGas.setTo: $to length must be >= 42');
        }
    ...
    public function setAmount(string $amount) {
        if ((!preg_match("/^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/", $amount))) {
            throw new IAE('OneEstimateGas.setAmount: $amount must match /^[+]?((\\d+(\\.\\d*)?)|(\\.\\d+))$/, ' . var_export($amount, true) . ' given');
        }
