 ...
 * MintErc721Celo Model
 */
class MintErc721Celo extends AbstractModel {

    public const DISCRIMINATOR = null;
    public const FEE_CURRENCY_CELO = 'CELO';
    public const FEE_CURRENCY_CUSD = 'CUSD';
    public const FEE_CURRENCY_CEUR = 'CEUR';
    protected static $_name = "MintErc721Celo";
    ...
    public function __construct(array $data = []) {
        foreach(["token_id"=>null, "to"=>null, "contract_address"=>null, "url"=>null, "from_private_key"=>null, "nonce"=>null, "fee_currency"=>null] as $k => $v) {
    ...
    public function setTokenId(string $token_id) {
        if ((mb_strlen($token_id) > 32)) {
            throw new IAE('MintErc721Celo.setTokenId: $token_id length must be <= 32');
        }
    ...
    public function setTo(string $to) {
        if ((mb_strlen($to) > 42)) {
            throw new IAE('MintErc721Celo.setTo: $to length must be <= 42');
        }
        if ((mb_strlen($to) < 42)) {
            throw new IAE('MintErc721Celo.setTo: $to length must be >= 42');
        }
    ...
    public function setContractAddress(string $contract_address) {
        if ((mb_strlen($contract_address) > 42)) {
            throw new IAE('MintErc721Celo.setContractAddress: $contract_address length must be <= 42');
        }
        if ((mb_strlen($contract_address) < 42)) {
            throw new IAE('MintErc721Celo.setContractAddress: $contract_address length must be >= 42');
        }
    ...
    public function setUrl(string $url) {
        if ((mb_strlen($url) > 256)) {
            throw new IAE('MintErc721Celo.setUrl: $url length must be <= 256');
        }
        $this->_data['url'] = $url;
    ...
    public function setFromPrivateKey(string $from_private_key) {
        if ((mb_strlen($from_private_key) > 66)) {
            throw new IAE('MintErc721Celo.setFromPrivateKey: $from_private_key length must be <= 66');
        }
        if ((mb_strlen($from_private_key) < 66)) {
            throw new IAE('MintErc721Celo.setFromPrivateKey: $from_private_key length must be >= 66');
        }
    ...
    public function setNonce(?float $nonce) {
        if (!is_null($nonce) && ($nonce < 0)) {
            throw new IAE('MintErc721Celo.setNonce: $nonce must be >=0');
        }
    ...
    public function setFeeCurrency(string $fee_currency) {
        ...
        if (!in_array($fee_currency, $allowed, true)) {
            throw new IAE(sprintf("MintErc721Celo.setFeeCurrency: fee_currency invalid value '%s', must be one of '%s'", $fee_currency, implode("', '", $allowed)));
        }
