...
use Tatum\Cryptography\Elliptic\Curve\PresetCurve;
...
class EC {
    ...
    public function genKeyPair($options = null) {
        ...
        $drbg = new HmacDRBG([
            ...
            "entropy" => isset($options["entropy"])
                ? $options["entropy"]
                : Utils::randBytes($this->hash["hmacStrength"]),
            "nonce" => $this->n->toArray()
        ]);

        $bytes = $this->n->byteLength();
        $ns2 = $this->n->sub(new BigNumber(2));
        while (true) {
            $priv = new BigNumber($drbg->generate($bytes));
            if ($priv->cmp($ns2) > 0) {
                continue;
            }
    ...
    public function sign($msg, $key, $enc = null, $options = null) {
        ...
        $msg = $this->_truncateToN(new BigNumber($msg, 16));
        ...
        if (isset($options["k"])) {
            ...
            $kFunc = function ($iter) use ($drbg, $bytes) {
                return new BigNumber($drbg->generate($bytes));
            };
        }

        // Number of bytes to generate
        $ns1 = $this->n->sub(new BigNumber(1));
