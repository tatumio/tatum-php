...
use Tatum\Cryptography\Elliptic\Curve\ShortCurve\Point;
use Tatum\Cryptography\Elliptic\Curve\ShortCurve\JPoint;
...
class ShortCurve extends BaseCurve {
    ...
    private function _getEndomorphism($conf) {
        ...
        if (!isset($conf["basis"])) {
            ...
            $callback = function ($vector) {
                return [
                    "a" => new BigNumber($vector["a"], 16),
                    "b" => new BigNumber($vector["b"], 16)
                ];
            };
    ...
    private function _getEndoRoots($num) {
        ...
        // Root = (-1 +- Sqrt(-3)) / 2
        //
        $red = $num === $this->p ? $this->red : BigNumber::mont($num);
        $tinv = (new BigNumber(2))->toRed($red)->redInvm();
        $ntinv = $tinv->redNeg();
