...
abstract class BaseCurve {
     ...
     * @param array $json
     * @param Red   $red
     * @return \Tatum\Cryptography\Elliptic\Curve\BaseCurve\Point
     */
    abstract function pointFromJSON($json, $red = null): \Tatum\Cryptography\Elliptic\Curve\BaseCurve\Point;

    /**
     * Compatiblity with short point
     *
     * @return \Tatum\Cryptography\Elliptic\Curve\BaseCurve\Point
     */
    abstract function jpoint($x, $y, $z, $t = null): \Tatum\Cryptography\Elliptic\Curve\BaseCurve\Point;
    ...
    function __construct($type, $conf) {
        ...
        $this->red = isset($conf["prime"]) ? BigNumber::red($conf["prime"]) : BigNumber::mont($this->p);
        ...
        $this->n = isset($conf["n"]) ? new BigNumber($conf["n"], 16) : null;
        $this->g = isset($conf["g"])
            ? $this->pointFromJSON($conf["g"], isset($conf["gRed"]) ? $conf["gRed"] : null)
            : null;
    ...
    public function _wnafMulAdd($defW, $points, $coeffs, $len, $jacobianResult = false) {
        ...
        for ($i = $len - 1; $i >= 1; $i -= 2) {
            ...
            $comb = [$points[$a] /* 1 */, null /* 3 */, null /* 5 */, $points[$b] /* 7 */];
