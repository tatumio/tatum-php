...
use Tatum\Cryptography\Exceptions\SignatureNotCanonical;
...
abstract class CheckerBase {
    ...
    public function checkSignatureEncoding(BufferInterface $signature, int $flags) {
        ...
        if (
            ($flags & (Interpreter::VERIFY_DERSIG | Interpreter::VERIFY_LOW_S | Interpreter::VERIFY_STRICTENC)) !== 0 &&
            !$this->isValidSignatureEncoding($signature)
        ) {
            throw new ScriptRuntimeException(Interpreter::VERIFY_DERSIG, "Signature with incorrect encoding");
    ...
    public function checkSig(
        ...
        BufferInterface $sigBuf,
        BufferInterface $keyBuf,
        int $sigVersion,
        int $flags
    ) {
    ...
    /**
     * @param \Tatum\Cryptography\Script\Interpreter\Number $scriptLockTime
     * @return bool
     */
    public function checkLockTime(\Tatum\Cryptography\Script\Interpreter\Number $scriptLockTime): bool {
    ...
    /**
     * @param \Tatum\Cryptography\Script\Interpreter\Number $sequence
     * @return bool
     */
    public function checkSequence(\Tatum\Cryptography\Script\Interpreter\Number $sequence): bool {
        ...
        $mask =
            TransactionInputInterface::SEQUENCE_LOCKTIME_TYPE_FLAG | TransactionInputInterface::SEQUENCE_LOCKTIME_MASK;

        $txToSequenceMasked = $txSequence & $mask;
        $nSequenceMasked = $sequence->getInt() & $mask;
        if (
            !(
                ($txToSequenceMasked < TransactionInput::SEQUENCE_LOCKTIME_TYPE_FLAG &&
                    $nSequenceMasked < TransactionInput::SEQUENCE_LOCKTIME_TYPE_FLAG) ||
                ($txToSequenceMasked >= TransactionInput::SEQUENCE_LOCKTIME_TYPE_FLAG &&
                    $nSequenceMasked >= TransactionInput::SEQUENCE_LOCKTIME_TYPE_FLAG)
            )
        ) {
