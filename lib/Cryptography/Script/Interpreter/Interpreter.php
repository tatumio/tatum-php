...
class Interpreter implements InterpreterInterface {
    /**
     * @var \Tatum\Cryptography\Math\Math
     */
    private $math;
     ...
     * @return bool
     */
    private function verifyWitnessProgram(
            ...
            foreach ($parser as $operation) {
                ...
                if ($fExec && $operation->isPush()) {
                    ...
                    switch ($opCode) {
                        ...
                        case Opcodes::OP_15:
                        case Opcodes::OP_16:
                            $num = \Tatum\Cryptography\Script\Script::decodeOpN($opCode);
                            ...
                            if (!($flags & self::VERIFY_CHECKLOCKTIMEVERIFY)) {
                                if ($flags & self::VERIFY_DISCOURAGE_UPGRADABLE_NOPS) {
                                    throw new ScriptRuntimeException(
                                        self::VERIFY_DISCOURAGE_UPGRADABLE_NOPS,
                                        "Upgradable NOP found - this is discouraged"
                                    );
                                }
                                break;
                            }
                        ...
                        case Opcodes::OP_CHECKSEQUENCEVERIFY:
                            ...
                            if (
                                $this->math->cmp(
                                    $this->math->bitwiseAnd(
                                        $nSequence,
                                        gmp_init(TransactionInputInterface::SEQUENCE_LOCKTIME_DISABLE_FLAG, 10)
                                    ),
                                    $zero
                                ) !== 0
                            ) {
                        ...
                        case Opcodes::OP_NOTIF:
                            ...
                            if ($fExec) {
                                ...
                                if ($sigVersion === SigHash::V1 && $flags & self::VERIFY_MINIMALIF) {
                                    if ($vch->getSize() > 1) {
                                        throw new ScriptRuntimeException(
                                            self::VERIFY_MINIMALIF,
                                            "Input to OP_IF/NOTIF should be minimally encoded"
                                        );
                                    }
                        ...
                        case Opcodes::OP_ROLL:
                            ...
                            $mainStack->pop();
                            if (
                                $this->math->cmp($n, $zero) < 0 ||
                                $this->math->cmp($n, gmp_init(count($mainStack))) >= 0
                            ) {
                                throw new \RuntimeException("Invalid stack operation OP_PICK");
                            }
                        ...
                        case $opCode >= Opcodes::OP_ADD && $opCode <= Opcodes::OP_MAX:
                            ...
                            if ($opCode === Opcodes::OP_ADD) {
                                ...
                                $num =
                                    (int) ($this->math->cmp($num1, $zero) !== 0 &&
                                        $this->math->cmp($num2, $zero) !== 0);
                        ...
                        case Opcodes::OP_CHECKSIGVERIFY:
                            ...
                            if (!$success && $flags & self::VERIFY_NULLFAIL && $vchSig->getSize() > 0) {
                                throw new ScriptRuntimeException(
                                    self::VERIFY_NULLFAIL,
                                    "Signature must be zero for failed OP_CHECK(MULTIS)SIG operation"
                                );
                            }
                        ...
                        case Opcodes::OP_CHECKMULTISIGVERIFY:
                            ...
                            while ($i-- > 1) {
                                ...
                                if (
                                    !$fSuccess &&
                                    $flags & self::VERIFY_NULLFAIL &&
                                    !$ikey2 &&
                                    $mainStack[-1]->getSize() > 0
                                ) {
                                    throw new ScriptRuntimeException(
                                        self::VERIFY_NULLFAIL,
                                        "Bad signature must be empty vector"
                                    );
                                }
                            ...
                            if ($flags & self::VERIFY_NULL_DUMMY && $mainStack[-1]->getSize() !== 0) {
                                throw new ScriptRuntimeException(
                                    self::VERIFY_NULL_DUMMY,
                                    "Extra P2SH stack value should be OP_0"
                                );
                            }
