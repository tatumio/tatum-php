...
class Script extends Serializable implements ScriptInterface {
    ...
    public static function decodeOpN(int $op): int {
        if ($op === Opcodes::OP_0) {
            return 0;
        }
    ...
    public static function encodeOpN(int $op): int {
        if ($op === 0) {
            return Opcodes::OP_0;
        }
     ...
     * @return int
     */
    public function countWitnessSigOps(
    ...
    public function isPushOnly(array &$ops = null): bool {
        ...
        foreach ($decoded as $entity) {
            ...
            if ($op >= Opcodes::OP_1 && $op <= Opcodes::OP_16) {
                $data[] = Number::int(self::decodeOpN($op))->getBuffer();
            } else {
                $data[] = $entity->getData();
            }
        }
    ...
    public function isWitness(&$program = null): bool {
        ...
        if ($script[1]->isPush() && $size === $witness->getDataSize() + 2) {
            $program = new WitnessProgram(self::decodeOpN($version), $witness->getData());
            return true;
        }
    ...
    public function isP2SH(&$scriptHash): bool {
        if (
            ...
            ($this->script[0] =
                Opcodes::OP_HASH160 && ($this->script[1] = 20 && ($this->script[22] = Opcodes::OP_EQUAL)))
        ) {
