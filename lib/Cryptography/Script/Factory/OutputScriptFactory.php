...
class OutputScriptFactory {
    ...
    public function multisig(int $m, array $keys = [], bool $sort = true): ScriptInterface {
        return self::multisigKeyBuffers(
            $m,
            array_map(function (PublicKeyInterface $key): BufferInterface {
    ...
    public function multisigKeyBuffers(int $m, array $keys = [], bool $sort = true): ScriptInterface {
        ...
        foreach ($keys as $key) {
            if (
                $key->getSize() !== PublicKey::LENGTH_COMPRESSED &&
                $key->getSize() !== PublicKey::LENGTH_UNCOMPRESSED
            ) {
    ...
    public function witnessCoinbaseCommitment(BufferInterface $commitment): ScriptInterface {
        ...
        return ScriptFactory::sequence([Opcodes::OP_RETURN, new Buffer("\xaa\x21\xa9\xed" . $commitment->getBinary())]);
    }
}
