...
class Multisig {
    ...
    public function __construct(
        ...
        if (null === $pubKeySerializer) {
            $pubKeySerializer = EcSerializer::getSerializer(
     ...
     * @return Multisig
     */
    public static function fromDecodedScript(
        array $decoded,
        PublicKeySerializerInterface $pubKeySerializer = null,
        $allowVerify = false
    ) {
        ...
        $requiredSigs = \Tatum\Cryptography\Script\Script::decodeOpN($mCode);
        $publicKeyBuffers = [];
        foreach (array_slice($decoded, 1, -2) as $key) {
            /** @var \Tatum\Cryptography\Script\Parser\Operation $key */
            if (!$key->isPush()) {
        ...
        $keyCount = \Tatum\Cryptography\Script\Script::decodeOpN($nCode);
