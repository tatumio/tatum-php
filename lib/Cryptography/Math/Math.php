...
namespace Tatum\Cryptography\Math;

use Tatum\Cryptography\Ecc\Math\GmpMath;
...
class Math extends GmpMath {
    ...
    public function decodeCompact($compact, &$isNegative, &$isOverflow): \GMP {
        ...
        $zero = gmp_init(0);
        $isNegative =
            $this->cmp($word, $zero) !== 0 &&
            $this->cmp($this->bitwiseAnd($compact, gmp_init(0x00800000)), $zero) === 1;
        $isOverflow =
            $this->cmp($word, $zero) !== 0 &&
