...
namespace Tatum\Cryptography\Bloom;
...
use Tatum\Cryptography\Serializer\Bloom\BloomFilterSerializer;
...
class BloomFilter extends Serializable {
    ...
    public static function create(Math $math, int $nElements, float $nFpRate, int $nTweak, int $flags): BloomFilter {
        ...
        return new self($math, self::emptyFilter($size), self::idealNumHashFuncs($size, $nElements), $nTweak, $flags);
    }
    ...
    public static function idealSize(int $nElements, float $fpRate): int {
        return (int) floor(
            bcdiv(
                min(
                    bcmul(bcmul(bcdiv("-1", (string) self::LN2SQUARED), (string) $nElements), (string) log($fpRate)),
                    bcmul((string) self::MAX_FILTER_SIZE, "8")
                ),
                "8"
            )
        );
    ...
    public static function idealNumHashFuncs(int $filterSize, int $nElements) {
        return (int) floor(
            min(
                bcmul(bcdiv(bcmul((string) $filterSize, "8"), (string) $nElements), (string) self::LN2),
                bcmul((string) self::MAX_FILTER_SIZE, "8")
            )
        );
    }
