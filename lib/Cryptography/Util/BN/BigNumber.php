...
use \Exception;
use Tatum\Cryptography\Util\BN\BigInteger;
...
class BigNumber implements JsonSerializable {
    ...
    public function iuor(BigNumber $num) {
        $this->bi = $this->bi->binaryOr($num->bi);
        return $this;
    }

    public function ior(BigNumber $num) {
    ...
    // Or `num` with `this`
    public function _or(BigNumber $num) {
        if ($this->ucmp($num) > 0) {
    ...
    public function uor(BigNumber $num) {
        if ($this->ucmp($num) > 0) {
    ...
    public function iuand(BigNumber $num) {
        $this->bi = $this->bi->binaryAnd($num->bi);
        return $this;
    }

    public function iand(BigNumber $num) {
        if (assert_options(ASSERT_ACTIVE)) {
            assert(!$this->negative() && !$num->negative());
        }
    ...
    // And `num` with `this`
    public function _and(BigNumber $num) {
        if ($this->ucmp($num) > 0) {
    ...
    public function uand(BigNumber $num) {
        if ($this->ucmp($num) > 0) {
    ...
    public function iuxor(BigNumber $num) {
        $this->bi = $this->bi->binaryXor($num->bi);
        return $this;
    }

    public function ixor(BigNumber $num) {
        if (assert_options(ASSERT_ACTIVE)) {
            assert(!$this->negative() && !$num->negative());
        }
    ...
    // Xor `num` with `this`
    public function _xor(BigNumber $num) {
        if ($this->ucmp($num) > 0) {
    ...
    public function uxor(BigNumber $num) {
        if ($this->ucmp($num) > 0) {
    ...
    public function iadd(BigNumber $num) {
        $this->bi = $this->bi->add($num->bi);
        return $this;
    }

    // Add `num` to `this`
    public function add(BigNumber $num) {
        return $this->_clone()->iadd($num);
    }

    // Subtract `num` from `this` in-place
    public function isub(BigNumber $num) {
        $this->bi = $this->bi->sub($num->bi);
        return $this;
    }

    // Subtract `num` from `this`
    public function sub(BigNumber $num) {
        return $this->_clone()->isub($num);
    }

    // Multiply `this` by `num`
    public function mul(BigNumber $num) {
        return $this->_clone()->imul($num);
    }

    // In-place Multiplication
    public function imul(BigNumber $num) {
    ...
    // Math.pow(`this`, `num`)
    public function pow(BigNumber $num) {
        $res = clone $this;
    ...
    public function imaskn($bits) {
        ...
        return $this->iand(new BigNumber($mask, 2));
    }
    ...
    // Find `this` / `num`
    public function div(BigNumber $num) {
        if (assert_options(ASSERT_ACTIVE)) {
            assert(!$num->isZero());
        }
        $res = clone $this;
    ...
    // Find `this` % `num`
    public function mod(BigNumber $num) {
        if (assert_options(ASSERT_ACTIVE)) {
            assert(!$num->isZero());
        }
        $res = clone $this;
    ...
    public function umod(BigNumber $num) {
        if (assert_options(ASSERT_ACTIVE)) {
            assert(!$num->isZero());
        }
    ...
    // Find Round(`this` / `num`)
    public function divRound(BigNumber $num) {
        if (assert_options(ASSERT_ACTIVE)) {
            assert(!$num->isZero());
        }
    ...
    public function gcd(BigNumber $num) {
        $res = clone $this;
        $res->bi = $this->bi->gcd($num->bi);
        return $res;
    }

    public function invm(BigNumber $num) {
        $res = clone $this;
    ...
    public function bincn($num) {
        $tmp = (new BigNumber(1))->iushln($num);
        return $this->add($tmp);
    }
    ...
    // 0 - if `this` == `num`
    // -1 - if `this` < `num`
    public function cmp(BigNumber $num) {
        return $this->bi->cmp($num->bi);
    }

    public function ucmp(BigNumber $num) {
    ...
    public function gt(BigNumber $num) {
    ...
    public function gte(BigNumber $num) {
        return $this->cmp($num) >= 0;
    }
    ...
    public function lt(BigNumber $num) {
        return $this->cmp($num) < 0;
    }
    ...
    public function lte(BigNumber $num) {
        return $this->cmp($num) <= 0;
    }
    ...
    public function eq(BigNumber $num) {
        return $this->cmp($num) === 0;
    }
    ...
    public function redAdd(BigNumber $num) {
        if ($this->red === null) {
    ...
    public function redIAdd(BigNumber $num) {
        if ($this->red === null) {
    ...
    public function redSub(BigNumber $num) {
        if ($this->red === null) {
    ...
    public function redISub(BigNumber $num) {
        if ($this->red === null) {
    ...
    public function redShl(BigNumber $num) {
        if ($this->red === null) {
    ...
    public function redMul(BigNumber $num) {
        if ($this->red === null) {
    ...
    public function redIMul(BigNumber $num) {
        if ($this->red === null) {
    ...
    public function redPow(BigNumber $num) {
        if (null !== $this->red) {
    ...
    public function inspect() {
        return ($this->red == null ? "<BigNumber: " : "<BigNumber-R: ") . $this->toString(16) . ">";
    }

    public function __debugInfo() {
        if ($this->red != null) {
            return ["BigNumber-R" => $this->toString(16)];
        } else {
            return ["BigNumber" => $this->toString(16)];
        }
    }
}
