...
use Tatum\Cryptography\Semver\Constraint\ConstraintInterface;
use Tatum\Cryptography\Semver\Constraint\EmptyConstraint;
use Tatum\Cryptography\Semver\Constraint\MultiConstraint;
...
class VersionParser {
    ...
    public function parseConstraints($constraints) {
        ...
        if (1 === count($orGroups)) {
            ...
            ($a = (string) $orGroups[0]) &&
            strpos($a, "[>=") === 0 &&
            false !== ($posA = strpos($a, "<", 4)) &&
            ($b = (string) $orGroups[1]) &&
            strpos($b, "[>=") === 0 &&
            false !== ($posB = strpos($b, "<", 4)) &&
    ...
    private function parseConstraint($constraint) {
        ...
        if (
            preg_match(
                "{^(?P<from>" . $versionRegex . ") +- +(?P<to>" . $versionRegex . ')($)}i',
                $constraint,
                $matches
            )
        ) {
