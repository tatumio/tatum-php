...
class TransactionInput extends Serializable implements TransactionInputInterface {
    ...
    public function __construct(
        ...
        int $sequence = self::SEQUENCE_FINAL
    ) {
    ...
    public function isCoinbase(): bool {
        ...
        return $outpoint->getTxId()->getBinary() === str_pad("", 32, "\x00") && $outpoint->getVout() == 0xffffffff;
    }
    ...
    public function isLockedToTime(): bool {
        return !$this->isSequenceLockDisabled() &&
            ($this->sequence & self::SEQUENCE_LOCKTIME_TYPE_FLAG) === self::SEQUENCE_LOCKTIME_TYPE_FLAG;
    }
    ...
    public function getRelativeTimeLock(): int {
        if (!$this->isLockedToTime()) {
            throw new \RuntimeException(
                "Cannot decode time based locktime when disable flag set/timelock flag unset/tx is coinbase"
            );
        }
    ...
    public function getRelativeBlockLock(): int {
        if (!$this->isLockedToBlock()) {
            throw new \RuntimeException(
                "Cannot decode block locktime when disable flag set/timelock flag set/tx is coinbase"
            );
        }
