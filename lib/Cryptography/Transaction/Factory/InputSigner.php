...
use Tatum\Cryptography\Exceptions\ScriptRuntimeException;
use Tatum\Cryptography\Exceptions\SignerException;
use Tatum\Cryptography\Exceptions\UnsupportedScript;
use Tatum\Cryptography\Locktime;
use Tatum\Cryptography\Script\Classifier\OutputData;
use Tatum\Cryptography\Script\FullyQualifiedScript;
...
use Tatum\Cryptography\Script\Interpreter\Stack;
use Tatum\Cryptography\Script\Opcodes;
use Tatum\Cryptography\Script\Parser\Operation;
use Tatum\Cryptography\Script\Path\BranchInterpreter;
use Tatum\Cryptography\Script\ScriptFactory;
use Tatum\Cryptography\Script\ScriptInfo\Multisig;
use Tatum\Cryptography\Script\ScriptInfo\PayToPubkey;
use Tatum\Cryptography\Script\ScriptInfo\PayToPubkeyHash;
...
class InputSigner implements InputSignerInterface {
     ...
     * @var array
     */
    protected static $canSign = [ScriptType::P2PKH, ScriptType::P2PK, ScriptType::MULTISIG];
    ...
    public function __construct(
        ...
        $defaultFlags =
            Interpreter::VERIFY_DERSIG |
            Interpreter::VERIFY_P2SH |
            Interpreter::VERIFY_CHECKLOCKTIMEVERIFY |
            Interpreter::VERIFY_CHECKSEQUENCEVERIFY |
        ...
        $this->txSigSerializer =
            $sigSerializer ?:
            new TransactionSignatureSerializer(
     ...
     * @throws \Tatum\Cryptography\Exceptions\ScriptRuntimeException
     */
    private function sortMultisigs(
        ...
        if (!$mainStack->isEmpty()) {
            if (count($pathData) === 0) {
                throw new \RuntimeException(
                    "Extracted conditional op (including mainstack) without corresponding element in path data"
                );
            }
                ...
                foreach ($templateTypes as $k => $checksig) {
                    if ($fExec) {
                        if ($checksig instanceof Checksig) {
                            $this->extractChecksig(
                                $signScript->getScript(),
                                $checksig,
                                $stack,
                                $this->fqs->sigVersion(),
                                $resolvesFalse
    ...
    public function checkTimeLock(TimeLock $timelock) {
        ...
        if (($this->flags & Interpreter::VERIFY_CHECKLOCKTIMEVERIFY) != 0 && $info instanceof CheckLocktimeVerify) {
            ...
            if (!$this->signatureChecker->checkLockTime(Number::int($verifyLocktime))) {
                ...
                $requiredTime = $info->isLockedToBlock()
                    ? "block {$info->getLocktime()}"
                    : "{$info->getLocktime()}s (median time past)";
        ...
        if (($this->flags & Interpreter::VERIFY_CHECKSEQUENCEVERIFY) != 0 && $info instanceof CheckSequenceVerify) {
            ...
            if (!$this->signatureChecker->checkSequence(Number::int($info->getRelativeLockTime()))) {
                ...
                $cmp = $this->compareRangeAgainstThreshold(
                    $info->getRelativeLockTime(),
                    $input->getSequence(),
                    TransactionInput::SEQUENCE_LOCKTIME_TYPE_FLAG
                );
                if ($cmp === -1) {
                ...
                throw new \RuntimeException(
                    "Output unspendable with this sequence, must be locked for {$requiredLock}"
                );
            }
        }
    }
     ...
     * @throws \Exception
     */
    public function extractChecksig(
        ScriptInterface $script,
        Checksig $checksig,
        Stack $stack,
        int $sigVersion,
        ...
        if ($checksig->getType() === ScriptType::P2PKH) {
            ...
            if ($size > 0) {
                ...
                $value = false;
                if (!$expectFalse) {
                    $value = $this->signatureChecker->checkSig(
                        $script,
                        $vchSig,
                        $checksig->getSolution(),
                        $this->fqs->sigVersion(),
                        $this->flags
                    );
                    if (!$value) {
        ...
        } elseif (ScriptType::MULTISIG === $checksig->getType()) {
            /** @var Multisig $info */
     ...
     */
    public function calculateSigHashUnsafe(
        ScriptInterface $scriptCode,
        int $sigHashType,
        int $sigVersion
    ): BufferInterface {
    ...
    public function getSigHash(int $sigHashType): BufferInterface {
        return $this->calculateSigHashUnsafe(
            $this->fqs->signScript()->getScript(),
            $sigHashType,
            $this->fqs->sigVersion()
        );
    }
