...
use Tatum\Cryptography\FG\ASN1\Universal\Boolean;
use Tatum\Cryptography\FG\ASN1\Universal\Enumerated;
use Tatum\Cryptography\FG\ASN1\Universal\GeneralizedTime;
...
use Tatum\Cryptography\FG\ASN1\Universal\RelativeObjectIdentifier;
...
use Tatum\Cryptography\FG\ASN1\Universal\UTCTime;
use Tatum\Cryptography\FG\ASN1\Universal\IA5String;
use Tatum\Cryptography\FG\ASN1\Universal\PrintableString;
use Tatum\Cryptography\FG\ASN1\Universal\NumericString;
use Tatum\Cryptography\FG\ASN1\Universal\UTF8String;
use Tatum\Cryptography\FG\ASN1\Universal\UniversalString;
use Tatum\Cryptography\FG\ASN1\Universal\CharacterString;
use Tatum\Cryptography\FG\ASN1\Universal\GeneralString;
use Tatum\Cryptography\FG\ASN1\Universal\VisibleString;
use Tatum\Cryptography\FG\ASN1\Universal\GraphicString;
use Tatum\Cryptography\FG\ASN1\Universal\BMPString;
use Tatum\Cryptography\FG\ASN1\Universal\T61String;
use Tatum\Cryptography\FG\ASN1\Universal\ObjectDescriptor;
...
abstract class ASNObject implements Parsable {
    ...
    public function getIdentifier() {
        ...
        if (Identifier::isLongForm($firstOctet)) {
            throw new LogicException(
                sprintf(
                    'Identifier of %s uses the long form and must therefor override "ASNObject::getIdentifier()".',
                    get_class($this)
                )
            );
        }
     ...
     *
     * @return \Tatum\Cryptography\FG\ASN1\ASNObject
     */
    ...
    protected static function parseIdentifier($identifierOctet, $expectedIdentifier, $offsetForExceptionHandling) {
        ...
        if ($identifierOctet != $expectedIdentifier) {
            $message =
                "Can not create an " .
                Identifier::getName($expectedIdentifier) .
                " from an " .
                Identifier::getName($identifierOctet);
    ...
    protected static function parseContentLength(&$binaryData, &$offsetIndex, $minimumLength = 0) {
        ...
        if (($contentLength & 0x80) != 0) {
            ...
            if ($contentLength->compare(PHP_INT_MAX) > 0) {
                throw new ParserException(
                    "Can not parse content length from data: length > maximum integer",
                    $offsetIndex
                );
            }
        ...
        if ($contentLength < $minimumLength) {
            throw new ParserException(
                "A " .
                    get_called_class() .
                    " should have a content length of at least {$minimumLength}. Extracted length was {$contentLength}",
                $offsetIndex
            );
        }
        ...
        if ($lenDataRemaining < $contentLength) {
            throw new ParserException(
                "Content length {$contentLength} exceeds remaining data length {$lenDataRemaining}",
                $offsetIndex
            );
        }
