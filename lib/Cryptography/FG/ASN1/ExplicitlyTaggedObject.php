...
class ExplicitlyTaggedObject extends ASNObject {
    /** @var \Tatum\Cryptography\FG\ASN1\ASNObject[] */
    private $decoratedObjects;
    private $tag;

    /**
     * @param int $tag
     * @param \Tatum\Cryptography\FG\ASN1\ASNObject $objects,...
     */
    ...
    public static function fromBinary(&$binaryData, &$offsetIndex = 0) {
        ...
        assert(
            Identifier::isContextSpecificClass($firstIdentifierOctet),
            "identifier octet should indicate context specific class"
        );
        ...
        if ($remainingContentLength != 0) {
            throw new ParserException(
                "Context-Specific explicitly tagged object [$tag] starting at offset $offsetIndexOfDecoratedObject specifies a length of $totalContentLength octets but $remainingContentLength remain after parsing the content",
                $offsetIndexOfDecoratedObject
            );
        }
