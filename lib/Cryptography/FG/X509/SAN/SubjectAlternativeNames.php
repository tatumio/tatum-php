    ...
    public static function fromBinary(&$binaryData, &$offsetIndex = 0) {
        ...
        if ($contentLength < 2) {
            throw new ParserException(
                "Can not parse Subject Alternative Names: The Sequence within the octet string after the Object identifier " .
                    OID::CERT_EXT_SUBJECT_ALT_NAME .
                    " is too short ({$contentLength} octets)",
                $offsetIndex
            );
        }
        ...
        if ($sequence->getObjectLength() != $contentLength) {
            throw new ParserException(
                "Can not parse Subject Alternative Names: The Sequence length does not match the length of the surrounding octet string",
                $offsetIndex
            );
        }

        $parsedObject = new self();
        /** @var \Tatum\Cryptography\FG\ASN1\ASNObject $object */
        foreach ($sequence as $object) {
            if ($object->getType() == DNSName::IDENTIFIER) {
                ...
                throw new ParserException(
                    "Could not parse Subject Alternative Name: Only DNSName and IP SANs are currently supported",
                    $offsetIndex
                );
            }
        }
