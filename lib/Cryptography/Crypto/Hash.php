...
namespace Tatum\Cryptography\Crypto;
!defined("TATUM-SDK") && exit();

use Tatum\Cryptography\Buffertools\Buffer;
use Tatum\Cryptography\Buffertools\BufferInterface;
use Tatum\Cryptography\Murmur;
...
class Hash {
    ...
    public static function pbkdf2(
        string $algorithm,
        BufferInterface $password,
        BufferInterface $salt,
        int $count,
        int $keyLength
    ): BufferInterface {
        if ($keyLength < 0) {
        ...
        return new Buffer(
            \hash_pbkdf2($algorithm, $password->getBinary(), $salt->getBinary(), $count, $keyLength, true),
            $keyLength
        );
    }
