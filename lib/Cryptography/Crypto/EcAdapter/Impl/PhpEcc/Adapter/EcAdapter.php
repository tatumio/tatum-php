...
namespace Tatum\Cryptography\Crypto\EcAdapter\Impl\PhpEcc\Adapter;
!defined("TATUM-SDK") && exit();

use Tatum\Cryptography\Crypto\EcAdapter\Adapter\EcAdapterInterface;
use Tatum\Cryptography\Crypto\EcAdapter\Impl\PhpEcc\Key\PrivateKey;
...
class EcAdapter implements EcAdapterInterface {
    ...
    public function recover(BufferInterface $messageHash, CompactSignatureInterface $signature): PublicKeyInterface {
        ...
        $alpha = $math->mod(
            $math->add($math->add($math->pow($x, 3), $math->mul($curve->getA(), $x)), $curve->getB()),
            $curve->getPrime()
        );
        ...
        for ($i = 0; $i < 4; $i++) {
            try {
                $recover = $this->recover(
                    $messageHash,
                    new CompactSignature($this, $r, $s, $i, $publicKey->isCompressed())
                );
    ...
    public function publicKeyFromBuffer(BufferInterface $publicKey): PublicKeyInterface {
        ...
        return new PublicKey($this, $curve->getPoint($x, $y), $compressed, $prefix);
    }
}
