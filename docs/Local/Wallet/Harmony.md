...
```php
local()->wallet()->harmony()->generateWallet(
    ?string $mnemonic
): \Tatum\Model\Wallet
```
Generate wallet
...
try {
    ...
    echo 'Exception when calling local()->wallet()->harmony()->generateWallet(): ', $exc->getMessage(), PHP_EOL;
}
```
...
```php
local()->wallet()->harmony()->generateAddressFromXpub(
    ?string $xpub, 
    ?int $index
...
try {
    ...
    echo 'Exception when calling local()->wallet()->harmony()->generateAddressFromXpub(): ', $exc->getMessage(), PHP_EOL;
}
```
...
```php
local()->wallet()->harmony()->generateAddressFromPrivateKey(
...
try {
    ...
    echo 'Exception when calling local()->wallet()->harmony()->generateAddressFromPrivateKey(): ', $exc->getMessage(), PHP_EOL;
}
```
...
```php
local()->wallet()->harmony()->generatePrivateKey(
    ?string $mnemonic, 
    ?int $index
): \Tatum\Model\PrivKey
```
...
try {
    ...
    echo 'Exception when calling local()->wallet()->harmony()->generatePrivateKey(): ', $exc->getMessage(), PHP_EOL;
}
```
