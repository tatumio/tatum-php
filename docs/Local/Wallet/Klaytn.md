...
```php
local()->wallet()->klaytn()->generateWallet(
    ?string $mnemonic
): \Tatum\Model\Wallet
```
Generate wallet
...
```php
local()->wallet()->klaytn()->generateAddressFromXpub(
    ?string $xpub, 
    ?int $index
...
```php
local()->wallet()->klaytn()->generateAddressFromPrivateKey(
...
```php
local()->wallet()->klaytn()->generatePrivateKey(
    ?string $mnemonic, 
    ?int $index
): \Tatum\Model\PrivKey
```
