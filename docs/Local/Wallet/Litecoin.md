...
```php
local()->wallet()->litecoin()->generateWallet(
    ?string $mnemonic
): \Tatum\Model\Wallet
```
Generate wallet
...
```php
local()->wallet()->litecoin()->generateAddressFromXpub(
    ?string $xpub, 
    ?int $index
...
```php
local()->wallet()->litecoin()->generateAddressFromPrivateKey(
...
```php
local()->wallet()->litecoin()->generatePrivateKey(
    ?string $mnemonic, 
    ?int $index
): \Tatum\Model\PrivKey
```
