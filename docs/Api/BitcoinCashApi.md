...
<h4>5 credits per API call.</h4>
 ...
 <p>Broadcast signed transaction to Bitcoin Cash blockchain. This method is used internally from Tatum KMS or Tatum client libraries. It is possible to create custom signing mechanism and use this method only for broadcasting data to the blockchain.</p>
...
<h4>5 credits per API call.</h4>
 ...
 <p>Generate Bitcoin Cash deposit address from Extended public key. Deposit address is generated for the specific index - each extended public key can generate up to 2^31 addresses starting from index 0 until 2^31 - 1. Generates new format of address starting with bitcoincash: in case of mainnet, bchtest: in case of testnet..</p>
...
<p>Get Bitcoin Cash Block detail by block hash or height.</p>
...
<p>Get Bitcoin Cash Blockchain Information. Obtain basic info like testnet / mainnet version of the chain, current block number and it's hash.</p>
...
<p>Get Bitcoin Cash Block hash. Returns hash of the block to get the block detail.</p>
...
<p>Get Bitcoin Cash Transaction by transaction hash.</p>
...
<p>Get Bitcoin Cash Transaction by address. Limit is 50 transaction per response.</p>
...
<p><b>2 credits per API call</b></p>
 ...
 <p>Use this endpoint URL as an http-based JSON RPC driver to connect directly to the node provided by Tatum. To learn more about JSON RPC, visit <a href="https://github.com/gcash/bchd/blob/master/docs/json_rpc_api.md#Methods" target="_blank">Bitcoin Cash developers' guide</a>.</p>
...
<p><b>10 credits per API call</b></p>
 ...
 <p>When an UTXO is entered into a transaction, the whole UTXO amount is included and must be spent. For example, address A receives two transactions, T1 with 1 BCH and T2 with 2 BCH. A transaction that consumes the UTXOs from both T1 and T2 will have an available amount of 3 BCH to spend:
<code>1 BCH (from T1) + 2 BCH (from T2) = 3 BCH (to spend in total)</code></p>
